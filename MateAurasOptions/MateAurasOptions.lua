...
local L = MateAuras.L
local ADDON_NAME = "MateAurasOptions";

local displayButtons = {};
OptionsPrivate.displayButtons = displayButtons;

local spellCache = MateAuras.spellCache;
local savedVars = {};
...
function OptionsPrivate.DuplicateAura(data, newParent, massEdit, targetIndex)
  ...
  while(MateAuras.GetData(new_id)) do
    new_id = base_id .. num
    num = num + 1
  end
  ...
  newData.id = new_id
  newData.parent = nil
  newData.uid = MateAuras.GenerateUniqueID()
  if newData.controlledChildren then
    newData.controlledChildren = {}
  end
  MateAuras.Add(newData)
  MateAuras.NewDisplayButton(newData, massEdit)
  if(newParent or data.parent) then
    local parentId = newParent or data.parent
    local parentData = MateAuras.GetData(parentId)
    local index
    if targetIndex then
      index = targetIndex
...
loadedFrame:SetScript("OnEvent", function(self, event, addon)
  if (event == "ADDON_LOADED") then
    if(addon == ADDON_NAME) then
      db = MateAurasSaved;
      MateAurasOptionsSaved = MateAurasOptionsSaved or {};

      odb = MateAurasOptionsSaved;
...
local function commonParent(controlledChildren)
  ...
  for index, id in ipairs(controlledChildren) do
    local childData = MateAuras.GetData(id);
    local childButton = OptionsPrivate.GetDisplayButton(id)
...
local function CreateNewGroupFromSelection(regionType, resetChildPositions)
  ...
  MateAuras.DeepMixin(data, OptionsPrivate.Private.data_stub)
  data.internalVersion = MateAuras.InternalVersion()
  ...
  if (parent) then
    ...
    MateAuras.Add(parentData);
    OptionsPrivate.Private.AddParents(parentData)
    MateAuras.NewDisplayButton(data);
    MateAuras.UpdateGroupOrders(parentData);
    OptionsPrivate.ClearOptions(parentData.id);
  ...
  for index, childId in pairs(tempGroup.controlledChildren) do
    ...
    local oldParentData = MateAuras.GetData(oldParent)
    if (oldParent) then
      local oldIndex = childButton:GetGroupOrder()

      tremove(oldParentData.controlledChildren, oldIndex)
      MateAuras.Add(oldParentData)
      OptionsPrivate.Private.AddParents(oldParentData)
      MateAuras.UpdateGroupOrders(oldParentData);
      MateAuras.ClearAndUpdateOptions(oldParent);
...
function OptionsPrivate.MultipleDisplayTooltipMenu()
  ...
  local menu = {
    ...
    },
    {
      text = L["Close"],
      notCheckable = 1,
      func = function() MateAuras_DropDownMenu:Hide() end
    }
  };
  ...
  if (allSameParent and commonParent) then
    local parentData = MateAuras.GetData(commonParent);
...
StaticPopupDialogs["MateAuras_CONFIRM_DELETE"] = {
  text = "",
  button1 = L["Delete"],
  button2 = L["Cancel"],
...
function OptionsPrivate.IsWagoUpdateIgnored(auraId)
    local auraData = MateAuras.GetData(auraId)
      if auraData then
...
function OptionsPrivate.ConfirmDelete(toDelete, parents)
  if toDelete then
    ...
    StaticPopupDialogs["MateAuras_CONFIRM_DELETE"].text = warningForm:format(#toDelete)
    StaticPopup_Show("MateAuras_CONFIRM_DELETE", "", "", {toDelete = toDelete, parents = parents})
  end
end
...
if not MateAuras.ToggleOptions then
  ---@type fun(msg: string, Private: Private)
  function MateAuras.ToggleOptions(msg, Private)
    if not Private then
      return
    end
    ...
    elseif (InCombatLockdown()) then
      MateAuras.prettyPrint(L["Options will open after combat ends."])
      reopenAfterCombat = true;
    else
      MateAuras.ShowOptions(msg);
    end
  end
end

function MateAuras.HideOptions()
  if(frame) then
    frame:Hide()
  end
end
...
local function EnsureDisplayButton(data)
  ...
  if not(displayButtons[id]) then
    displayButtons[id] = AceGUI:Create("MateAurasDisplayButton");
    if(displayButtons[id]) then
      displayButtons[id]:SetData(data);
      displayButtons[id]:Initialize();
      displayButtons[id]:UpdateWarning()
    else
      print("|cFF8800FFMateAuras|r: Error creating button for", id);
    end
  end
end
  ...
  local func2 = function()
    ...
    for _, id in pairs(unloadedSorted) do
      ...
      if(data) then
        ...
        MateAuras.UpdateThumbnail(data);
  ...
  local func1 = function()
    ...
    if parents then
      for id in pairs(parents) do
        local parentData = MateAuras.GetData(id)
        ...
        MateAuras.Add(parentData)
        MateAuras.ClearAndUpdateOptions(parentData.id)
        parentButton:UpdateParentWarning()
...
function MateAuras.ShowOptions(msg)
  ...
  if (frame.pickedDisplay) then
    if (OptionsPrivate.IsPickedMultiple()) then
      ...
      MateAuras.PickDisplay(frame.pickedDisplay);
    end
  else
    frame:NewAura();
  end
...
function MateAuras.ClearAndUpdateOptions(id, clearChildren)
...
function MateAuras.FillOptions()
  frame:FillOptions()
end
...
function MateAuras.NewDisplayButton(data, massEdit)
  local id = data.id;
  OptionsPrivate.Private.ScanForLoads({[id] = true});
  EnsureDisplayButton(db.displays[id]);
  MateAuras.UpdateThumbnail(db.displays[id]);
...
function MateAuras.UpdateGroupOrders(data)
  if(data.controlledChildren) then
...
function OptionsPrivate.SortDisplayButtons(filter, overrideReset, id)
  ...
  if OptionsPrivate.Private.CompanionData.stash then
    for id, companionData in pairs(OptionsPrivate.Private.CompanionData.stash) do
      ...
      if frame.pendingInstallButton:GetExpanded() then
        if not child then
          child = AceGUI:Create("MateAurasPendingInstallButton")
  ...
  if OptionsPrivate.Private.CompanionData.slugs then
    ...
    for id, aura in pairs(MateAurasSaved.displays) do
          ...
          if auraData and auraData.wagoVersion then
            if tonumber(auraData.wagoVersion) > tonumber(version) then
              ...
              if frame.pendingUpdateButton:GetExpanded() then
                ...
                if not child then
                  child = AceGUI:Create("MateAurasPendingUpdateButton")
  ...
  for _, id in ipairs(topLevelLoadedAuras) do
    for child in OptionsPrivate.Private.TraverseLeafsOrAura(MateAuras.GetData(id)) do
...
function MateAuras.PickDisplay(id, tab, noHide)
...
function OptionsPrivate.PickDisplayMultipleShift(target)
  if (frame.pickedDisplay) then
    ...
    if (first and first ~= target) then
      ...
      local firstData = MateAuras.GetData(first);
      local targetData = MateAuras.GetData(target);
      if (firstData.parent == targetData.parent and not targetData.controlledChildren and not firstData.controlledChildren) then
        ...
        if (firstData.parent) then
          local group = MateAuras.GetData(targetData.parent);
          for index, child in ipairs(group.controlledChildren) do
            ...
            if (child == target or child == first) then
              ...
              for i = index + 1, #group.controlledChildren do
                ...
                if (MateAuras.GetData(current).controlledChildren) then
                  -- Skip sub groups
                else
        ...
        elseif (firstData.parent == nil and targetData.parent == nil) then
          ...
          for index, button in ipairs(frame.buttonsScroll.children) do
            if button.type == "MateAurasDisplayButton" then
...
local function CompareButtonOrder(a, b)
  ...
  while(aNode) do
    local parent = MateAuras.GetData(aNode).parent
    if (parent) then
  ...
  while(bNode) do
    local parent = MateAuras.GetData(bNode).parent
    if parent then
...
function OptionsPrivate.DropIndicator()
  ...
  if not indicator then
    ---@class Frame
    indicator = CreateFrame("Frame", "MateAuras_DropIndicator")
    ...
    groupTexture:SetTexture("Interface\\AddOns\\MateAuras\\Media\\Textures\\Square_FullWhite")
...
function MateAuras.UpdateThumbnail(data)
  local id = data.id
...
function OptionsPrivate.OpenModelPicker(baseObject, path)
  if not(C_AddOns.IsAddOnLoaded("MateAurasModelPaths")) then
    local loaded, reason = C_AddOns.LoadAddOn("MateAurasModelPaths");
    if not(loaded) then
      reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
      MateAuras.prettyPrint(string.format(L["ModelPaths could not be loaded, the addon is %s"], reason));
      MateAuras.ModelPaths = {};
    end
    OptionsPrivate.ModelPicker(frame).modelTree:SetTree(MateAuras.ModelPaths)
  end
...
function OptionsPrivate.OpenTriggerTemplate(data, targetId)
  if not(C_AddOns.IsAddOnLoaded("MateAurasTemplates")) then
    local loaded, reason = C_AddOns.LoadAddOn("MateAurasTemplates");
    if not(loaded) then
      reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
      MateAuras.prettyPrint(string.format(L["Templates could not be loaded, the addon is %s"], reason));
      return;
    end
    frame.newView = MateAuras.CreateTemplateView(OptionsPrivate.Private, frame);
  end
...
function OptionsPrivate.UpdateTextReplacements(frame, data)
  ...
  -- Create a modified MateAurasSnippetButton for each property and add it to ScrollList
  ...
  for i, prop in ipairs(sortedProps) do
    if prop.type == "header" then
      ...
      local button = AceGUI:Create("MateAurasSnippetButton")
...
function MateAuras.SetMoverSizer(id)
...
function MateAuras.GetMoverSizerId()
...
function MateAuras.NewAura(sourceData, regionType, targetId)
  local function ensure(t, k, v)
    return t and k and v and t[k] == v
  end
  local new_id = OptionsPrivate.Private.FindUnusedId("New")
  local data = {id = new_id, regionType = regionType, uid = MateAuras.GenerateUniqueID()}
  MateAuras.DeepMixin(data, OptionsPrivate.Private.data_stub);
  if (sourceData) then
    MateAuras.DeepMixin(data, sourceData);
  end
  data.internalVersion = MateAuras.InternalVersion();
  OptionsPrivate.Private.validate(data, OptionsPrivate.Private.regionTypes[regionType].default);
  ...
  if targetId then
    ...
    if (target) then
      ...
      if (group) then
        ...
        MateAuras.Add(data);
        MateAuras.Add(group.data);
        OptionsPrivate.Private.AddParents(group.data)
        MateAuras.NewDisplayButton(data);
        MateAuras.UpdateGroupOrders(group.data);
      ...
      error(string.format("Calling 'MateAuras.NewAura' with invalid groupId %s. Reload your UI to fix the display list.", targetId))
    end
  else
    -- move source into the top-level list
    MateAuras.Add(data);
