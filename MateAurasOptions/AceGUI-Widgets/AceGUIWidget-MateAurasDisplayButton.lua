...
local Type, Version = "MateAurasDisplayButton", 60
  ...
  func = function()
    if (not IsRegionAGroup(clipboard.source) and IsRegionAGroup(clipboard.current)) then
      ...
      for child in OptionsPrivate.Private.TraverseLeafs(clipboard.current) do
        ...
        MateAuras.Add(child)
        MateAuras.ClearAndUpdateOptions(child.id)
      end
    else
      copyAuraPart(clipboard.source, clipboard.current, clipboard.part);
      MateAuras.Add(clipboard.current)
      MateAuras.ClearAndUpdateOptions(clipboard.current.id)
    end

    MateAuras.FillOptions()
    OptionsPrivate.Private.ScanForLoads({[clipboard.current.id] = true});
    OptionsPrivate.SortDisplayButtons(nil, true);
    MateAuras.PickDisplay(clipboard.current.id);
    MateAuras.UpdateThumbnail(clipboard.current.id);
    MateAuras.ClearAndUpdateOptions(clipboard.current.id);
  end
}
...
local Actions = {
    ...
    if source and not source.data.parent then
      if groupId then
        ...
        if group and group:IsGroup() then
          ...
          MateAuras.Add(source.data)
          MateAuras.Add(group.data)
          OptionsPrivate.Private.AddParents(group.data)
          MateAuras.UpdateGroupOrders(group.data)
          MateAuras.ClearAndUpdateOptions(group.data.id)
          MateAuras.ClearAndUpdateOptions(source.data.id)
  ...
  ["Ungroup"] =  function(source)
    if source and source.data.parent then
      local parent = MateAuras.GetData(source.data.parent)
      ...
      if ensure(children, index, source.data.id) then
        ...
        MateAuras.Add(parent);
        OptionsPrivate.Private.AddParents(parent)
        MateAuras.UpdateGroupOrders(parent);
        MateAuras.ClearAndUpdateOptions(parent.id);
...
local function IsParentRecursive(needle, parent)
  ...
  if needle.parent then
    local needleParent = MateAuras.GetData(needle.parent)
...
local methods = {
  ...
  ["Initialize"] = function(self)
      ...
      elseif(IsShiftKeyDown()) then
        ...
        if(editbox) then
          ...
          if not (GetCurrentRegion() == 5 or GetLocale() == "zhCN") then -- China region (5), and chinese locale profanity filter doesn't allow links in chat
            ...
            editbox:Insert("[MateAuras: "..fullName.." - "..self.data.id.."]"..url)
          else
            editbox:Insert("[MateAuras: "..fullName.." - "..self.data.id.."]")
          end
        ...
        if(mouseButton == "RightButton") then
          ...
          if(OptionsPrivate.IsDisplayPicked(self.data.id) and OptionsPrivate.IsPickedMultiple()) then
            LibDD:EasyMenu(OptionsPrivate.MultipleDisplayTooltipMenu(), MateAuras_DropDownMenu, self.frame, 0, 0, "MENU");
          else
            UpdateClipboardMenuEntry(self.data);
            LibDD:EasyMenu(self.menu, MateAuras_DropDownMenu, self.frame, 0, 0, "MENU");
            if not(OptionsPrivate.IsDisplayPicked(self.data.id)) then
              if self.data.controlledChildren then
                MateAuras.PickDisplay(self.data.id, "group")
              else
                MateAuras.PickDisplay(self.data.id);
              end
            end
          end
        else
    ...
    function self.callbacks.OnClickGrouping()
      if (MateAuras.IsImporting()) then return end;
      for index, selectedId in ipairs(self.grouping) do
        local selectedData = MateAuras.GetData(selectedId);
      ...
      MateAuras.FillOptions();
      MateAuras.UpdateGroupOrders(self.data);
    ...
    function self.callbacks.OnUpGroupClick()
      ...
      if(self.data.parent) then
        ...
        local parentData = MateAuras.GetData(self.data.parent);
        local index;
        ...
        if(index) then
          if(index <= 1) then
            ...
            OptionsPrivate.Private.Animate("button", MateAuras.GetData(parentData.controlledChildren[index-1]).uid, "main", updata, self.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            OptionsPrivate.Private.Animate("button", MateAuras.GetData(parentData.controlledChildren[index]).uid, "main", downdata, otherbutton.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            MateAuras.FillOptions()
          end
        else
          error("Display thinks it is a member of a group which does not control it");
      ...
      if(self.data.parent) then
        ...
        if(index) then
          if(index >= #parentData.controlledChildren) then
            ...
            local updata = {duration = 0.15, type = "custom", use_translate = true, x = 0, y = -32};
            local downdata = {duration = 0.15, type = "custom", use_translate = true, x = 0, y = 32};
            OptionsPrivate.Private.Animate("button", MateAuras.GetData(parentData.controlledChildren[index+1]).uid, "main", downdata, self.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            OptionsPrivate.Private.Animate("button", MateAuras.GetData(parentData.controlledChildren[index]).uid, "main", updata, otherbutton.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            MateAuras.FillOptions()
          end
        else
          error("Display thinks it is a member of a group which does not control it");
    ...
    function self.callbacks.OnRenameAction(newid)
      ...
      if not(newid == oldid) then
        MateAuras.Rename(self.data, newid);
      end
      self:UpdateParentWarning()
    end
    ...
    -- tinsert(self.menu, {
    --   text = L["|cFF8855FFMateAuras Sync|r"],
    --   notCheckable = true,
    --   func = function()
  ...
  ["Ungroup"] = function(self)
    ...
    local newParent = parentData.parent and MateAuras.GetData(parentData.parent)
    if newParent then
    ...
    self:UpdateIconsVisible()
    if newParent then
      MateAuras.Add(newParent)
      OptionsPrivate.Private.AddParents(newParent)
      MateAuras.ClearAndUpdateOptions(newParent.id)
      MateAuras.UpdateGroupOrders(newParent)
    end
    MateAuras.ClearAndUpdateOptions(self.data.id);
    ...
    if picked then
      ...
      if mode == "GROUP" then
        if self.data.regionType == "dynamicgroup" then
          ...
          local parentData = self.data.parent and MateAuras.GetData(self.data.parent)
  ...
  ["UpdateOffset"] = function(self)
    ...
    if group then
      ...
      while(group) do
        depth = depth + 1
        group = MateAuras.GetData(group).parent
      end
          ...
          onClick = function()
            MenuUtil.CreateContextMenu(UIParent, function(ownerRegion, root)
              root:CreateButton(soundText, function()
                MateAuras.PickDisplay(warning.auraId, tabsForWarning[warning.key] or "information")
              end)
  ...
  ["SetLoaded"] = function(self, prio, file, title, description)
    self:UpdateStatusIcon("load", prio, "Interface\\AddOns\\MateAuras\\Media\\Textures\\" .. file, title, description, nil)
    self:SortStatusIcons()
  end,
  ...
  ["OverrideIcon"] = function(self)
    self.icon:SetTexture("Interface\\Addons\\MateAuras\\Media\\Textures\\icon.blp")
    self.icon:Show()
    if(self.iconRegion and self.iconRegion.Hide) then
      self.iconRegion:Hide();
    end
...
local function Constructor()
  local name = "MateAurasDisplayButton"..AceGUI:GetNextWidgetNum(Type);
  ---@class Button
  ...
  renamebox:SetScript("OnEnterPressed", function()
    ...
    if(newid == "" or (newid ~= oldid and MateAuras.GetData(newid))) then
