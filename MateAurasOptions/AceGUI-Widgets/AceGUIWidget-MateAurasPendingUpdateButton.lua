...
local Type, Version = "MateAurasPendingUpdateButton", 6
...
local methods = {
  ...
  ["Initialize"] = function(self, id, companionData)
    ...
    function self.callbacks.OnUpdateClick()
      ...
      MateAuras.Import(self.companionData.encoded, nil, nil, linkedAuras)
    end
        ...
        if not self.linkedChildren[auraId] then
          tinsert(
            self.menu,
            {
              text = auraId,
              notCheckable = true,
              func = function() MateAuras.PickDisplay(auraId, "information") end
            }
          )
        end
      end
      LibDD:EasyMenu(self.menu, MateAuras_DropDownMenu, self.followLink, 0, 0, "MENU", 5)
    end
...
local function Constructor()
  local name = "MateAurasPendingUpdateButton" .. AceGUI:GetNextWidgetNum(Type)
  ...
  update:SetNormalTexture([[Interface\AddOns\MateAuras\Media\Textures\wagoupdate_refresh.tga]])
  update:Disable()
  update:SetWidth(24)
  update:SetHeight(24)
  ...
  tex:SetTexture([[Interface\AddOns\MateAuras\Media\Textures\wagoupdate_logo.tga]])
  tex:SetAllPoints()
  updateLogo.tex = tex
