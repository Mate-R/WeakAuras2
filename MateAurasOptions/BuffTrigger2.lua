...
local function getAuraMatchesLabel(name)
  local ids = MateAuras.spellCache.GetSpellsMatching(name)
  if ids then
    local numMatches = 0
...
local function getAuraMatchesList(name, showSpellIdRecommendation)
  ...
  if ids then
    ...
    for id, _ in pairs(ids) do
      ...
      if MateAuras.IsPlayerSpellOrOverridesAndBaseIsPlayerSpell(id) then
    ...
    if showSpellIdRecommendation then
      local tip = L["MateAuras recommends using spell ids instead of names. Spell ids are automatically localized."]
  ...
  for i = 1, size do
    ...
    if i ~= 1 then
      aura_options[prefix .. "space" .. i] = {
        ...
        width = MateAuras.normalWidth - 0.2,
    ...
    if isExactSpellId then
      aura_options[iconOption].name = function()
        return OptionsPrivate.Private.ExecEnv.GetSpellName(MateAuras.SafeToNumber(trigger[optionKey] and trigger[optionKey][i]) or "")
      end
      ...
      aura_options[iconOption].name = function()
        local spellId = trigger[optionKey] and trigger[optionKey][i] and MateAuras.SafeToNumber(trigger[optionKey][i])
        if spellId then
      ...
      aura_options[iconOption].desc = function()
        ...
        local spellId = input and MateAuras.SafeToNumber(input)
        if spellId then
      ...
      aura_options[iconOption].image = function()
        ...
        elseif input and input ~= "" then
          icon = "Interface\\AddOns\\MateAuras\\Media\\Textures\\info"
        end
        return icon and tostring(icon) or "", 18, 18
      end
      ...
      aura_options[iconOption].func = function()
        local input = trigger[optionKey] and trigger[optionKey][i]
        local spellId = input and MateAuras.SafeToNumber(trigger[optionKey][i])
        if spellId then
          -- Do nothing
        elseif input and input ~= "" then
          ...
          if bestSuggestion then
            ...
            MateAuras.Add(data)
            MateAuras.ClearAndUpdateOptions(data.id)
          end
        end
      end
    end
    ...
    aura_options[prefix .. i] = {
      ...
      get = function(info)
        ...
        local spellName, _, _, _, _, _, spellID = OptionsPrivate.Private.ExecEnv.GetSpellInfo(MateAuras.SafeToNumber(rawString))
        if spellName and spellID then
          return ("%s (%s)"):format(spellID, spellName) .. "\0" .. rawString
        elseif MateAuras.SafeToNumber(rawString) then
      ...
      set = function(info, v)
        ...
        if v == "" then
          ...
          if isExactSpellId then
            ...
            local _, spellId = MateAuras.spellCache.CorrectAuraName(v)
            if spellId then
      ...
      validate = isExactSpellId and MateAuras.ValidateNumeric or nil,
      control = "MateAurasInputFocus",
    }
  end
  -- VALIDATE ?
end
...
local function GetBuffTriggerOptions(data, triggernum)
  ...
  local ValidateNumeric = MateAuras.ValidateNumeric
  local aura_options = {
    useUnit = {
      type = "toggle",
      width = MateAuras.normalWidth,
    ...
    warnSpecifcUnit = {
      ...
      order = 10.4,
      hidden = function() return not (trigger.type == "aura2" and trigger.unit == "member" and MateAuras.UntrackableUnit(trigger.specificUnit)) end
    },
    warnSoftTarget = {
      type = "description",
      width = MateAuras.doubleWidth,
      ...
      order = 10.4,
      hidden = function() return not MateAuras.IsUntrackableSoftTarget(trigger.unit) end
    },
    useDebuffType = {
      type = "toggle",
      width = MateAuras.normalWidth,
    ...
    restrictions12_1Warning = {
      type = "description",
      width = MateAuras.doubleWidth,
      name = L["Since 12.1 when aura secrecy restrictions are active(combat or active m+), MateAuras can only track \"Never Secret\" auras by exact spell ID. Name(s), Ignored Name(s), and Ignored Exact Spell ID(s) will no longer work."],
      order = 11.16,
    },
    use_debuffClass = {
      type = "toggle",
    ...
    useActualSpec = {
      ...
      desc = L["Requires LibSpecialization, that is e.g. a up-to date MateAuras version"],
      order = 66.3,
      hidden = function() return
        not (trigger.type == "aura2" and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        and MateAuras.IsCataOrMistsOrRetail())
      end,
    },
    actualSpec = {
      type = "multiselect",
    ...
    useArenaSpec = {
      ...
      order = 67.8,
      hidden = function() return
        not (MateAuras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena")
      end
    },
    arena_spec = {
      type = "multiselect",
      ...
      values = OptionsPrivate.Private.spec_types_all,
      hidden = function()
        return not (MateAuras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena" and trigger.useArenaSpec)
      end,
      order = 67.9
    },
    arena_specSpace = {
      ...
      width = MateAuras.normalWidth,
      hidden = function()
        return not (MateAuras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena" and not trigger.useArenaSpec)
      end,
    },
    ...
    --   order = 69.81,
    --   width = MateAuras.doubleWidth,
    --   hidden = function() return not (trigger.type == "aura2" and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party") and MateAuras.IsRetail()) end
    -- },
    ignoreInvisible = {
      type = "toggle",
...
MateAuras.RegisterTriggerSystemOptions({"aura2"}, GetBuffTriggerOptions)
