...
function OptionsPrivate.CreateFrame()
  LibDD:Create_UIDropDownMenu("MateAuras_DropDownMenu", nil)
  local frame
  local db = OptionsPrivate.savedVars.db
  local odb = OptionsPrivate.savedVars.odb

  frame = CreateFrame("Frame", "MateAurasOptions", UIParent, "PortraitFrameTemplate")
  ...
  function OptionsPrivate.SetTitle(title)
    local text = "MateAuras " .. MateAuras.versionString
    if title and title ~= "" then
      text = ("%s - %s"):format(text, title)
    end
    MateAurasOptionsTitleText:SetText(text)
  end
  ...
  MateAurasOptionsPortrait:SetTexture([[Interface\AddOns\MateAuras\Media\Textures\logo_256_round.tga]])
  ...
  frame.UpdateFrameVisible = function(self)
    ...
    if self.minimized then
      MateAurasOptionsTitleText:Hide()
      ...
      MateAurasOptionsTitleText:Show()
  ...
  minimizebutton:SetPoint("RIGHT", frame.CloseButton, "LEFT", MateAuras.IsClassicOrWrathOrCataOrMists() and 10 or 0, 0)
  ...
  local tipPopup = CreateFrame("Frame", nil, frame, "BackdropTemplateMateAuras")
  ...
  local addFooter = function(title, texture, url, description, descriptionCJ, descriptionK, rightAligned, width)
    local button = AceGUI:Create("MateAurasToolbarButton")
  ...
  local discordButton = addFooter(L["Discord"], [[Interface\AddOns\MateAuras\Media\Textures\discord.tga]], "https://discord.gg/MateAuras",
            L["Chat with MateAuras experts on our Discord server."])
  discordButton:SetParent(tipFrame)
  discordButton:SetPoint("LEFT", tipFrame, "LEFT")

  local documentationButton = addFooter(L["Documentation"], [[Interface\AddOns\MateAuras\Media\Textures\GitHub.tga]], "https://github.com/Mate-R/MateAuras/wiki",
  ...
  local thanksButton = addFooter(L["Thanks"], [[Interface\AddOns\MateAuras\Media\Textures\waheart.tga]],
                                 "https://www.patreon.com/MateAuras", thanksList, thanksListCJ, thanksListK, nil, 800)
  ...
  local reportbugButton = addFooter(L["Found a Bug?"], [[Interface\AddOns\MateAuras\Media\Textures\bug_report.tga]], "https://github.com/Mate-R/MateAuras/issues/new?template=bug_report.yml",
  ...
  local wagoButton = addFooter(L["Find Auras"], [[Interface\AddOns\MateAuras\Media\Textures\wago.tga]], "https://wago.io",
  ...
  if not OptionsPrivate.Private.CompanionData.slugs then
    companionButton = addFooter(L["Update Auras"], [[Interface\AddOns\MateAuras\Media\Textures\wagoupdate_refresh.tga]], "https://MateAuras.wtf",
  ...
  container.frame:SetPoint("TOPLEFT", frame, "TOPRIGHT", -63 - MateAuras.normalWidth * 340, 0)
  ...
  -- filter line
  local filterInput = CreateFrame("EditBox", "MateAurasFilterInput", frame, "SearchBoxTemplate")
  ...
  local undo = AceGUI:Create("MateAurasToolbarButton")
  undo:SetText(L["Undo"])
  undo:SetTexture("Interface\\AddOns\\MateAuras\\Media\\Textures\\upleft")
  ...
  local redo = AceGUI:Create("MateAurasToolbarButton")
  redo:SetText(L["Redo"])
  redo:SetTexture("Interface\\AddOns\\MateAuras\\Media\\Textures\\upright")
  ...
  local newButton = AceGUI:Create("MateAurasToolbarButton")
  newButton:SetText(L["New Aura"])
  newButton:SetTexture("Interface\\AddOns\\MateAuras\\Media\\Textures\\newaura")
  ...
  local importButton = AceGUI:Create("MateAurasToolbarButton")
  importButton:SetText(L["Import"])
  importButton:SetTexture("Interface\\AddOns\\MateAuras\\Media\\Textures\\importsmall")
  ...
  local lockButton = AceGUI:Create("MateAurasToolbarButton")
  lockButton:SetText(L["Lock Positions"])
  lockButton:SetTexture("Interface\\AddOns\\MateAuras\\Media\\Textures\\lockPosition")
  lockButton:SetCallback("OnClick", function(self)
    if MateAurasOptionsSaved.lockPositions then
      lockButton:SetStrongHighlight(false)
      lockButton:UnlockHighlight()
      MateAurasOptionsSaved.lockPositions = false
    else
      lockButton:SetStrongHighlight(true)
      lockButton:LockHighlight()
      MateAurasOptionsSaved.lockPositions = true
    end
  end)
  if MateAurasOptionsSaved.lockPositions then
    lockButton:LockHighlight()
  ...
  local magnetButton = AceGUI:Create("MateAurasToolbarButton")
  magnetButton:SetText(L["Magnetically Align"])
  magnetButton:SetTexture("Interface\\AddOns\\MateAuras\\Media\\Textures\\magnetic")
  magnetButton:SetCallback("OnClick", function(self)
    if MateAurasOptionsSaved.magnetAlign then
      magnetButton:SetStrongHighlight(false)
      magnetButton:UnlockHighlight()
      MateAurasOptionsSaved.magnetAlign = false
    else
      magnetButton:SetStrongHighlight(true)
      magnetButton:LockHighlight()
      MateAurasOptionsSaved.magnetAlign = true
    end
  end)
  ...
  local pendingInstallButton = AceGUI:Create("MateAurasLoadedHeaderButton")
  ...
  local pendingUpdateButton = AceGUI:Create("MateAurasLoadedHeaderButton")
  ...
  -- Loaded section
  local loadedButton = AceGUI:Create("MateAurasLoadedHeaderButton")
  ...
  -- Not Loaded section
  local unloadedButton = AceGUI:Create("MateAurasLoadedHeaderButton")
  ...
  local sidegroup = AceGUI:Create("MateAurasInlineGroup")
  ...
  local dynamicTextCodesFrame = CreateFrame("Frame", "MateAurasTextReplacements", sidegroup.frame, "PortraitFrameTemplate")
  ...
  frame.UpdateOptions = function(self)
    ...
    if type(self.pickedDisplay) == "string" then
      data = MateAuras.GetData(frame.pickedDisplay)
    elseif self.pickedDisplay then
      data = tempGroup
    end
    ...
    if optionTable then
      AceConfigRegistry:RegisterOptionsTable("MateAuras", optionTable, true)
    end
  end
  ...
  frame.FillOptions = function(self)
    ...
    container.frame:SetPoint("TOPLEFT", frame, "TOPRIGHT", -63 - MateAuras.normalWidth * 340, -10)
    ...
    local group = AceGUI:Create("MateAurasInlineGroup")
  ...
  frame.NewAura = function(self)
    ...
    if self.pickedDisplay then
      if type(self.pickedDisplay) == "table" and tempGroup.controlledChildren and tempGroup.controlledChildren[1] then
        ...
        MateAuras.PickDisplay(targetId)
      elseif type(self.pickedDisplay) == "string" then
    ...
    if targetId then
      ...
      if pickedButton.data.controlledChildren then
        ...
        local parentData = parent and MateAuras.GetData(parent)
    ...
    if C_AddOns.GetAddOnEnableState("MateAurasTemplates") ~= Enum.AddOnEnableState.None then
      ...
      local button = AceGUI:Create("MateAurasNewButton")
    ...
    for index, regionType in ipairs(regionTypesSorted) do
      if (targetIsDynamicGroup and (regionType == "group" or regionType == "dynamicgroup")) then
        ...
        button:SetClick(function()
          MateAuras.NewAura(nil, regionType, self:GetTargetAura())
        end)
        containerScroll:AddChild(button)
      end
    end
    ...
    -- Import
    local importButton = AceGUI:Create("MateAurasNewButton")
  ...
  frame.PickDisplayMultiple = function(self, id)
    if not self.pickedDisplay then
      ...
      if type(self.pickedDisplay) == "string" then
        if MateAuras.GetData(self.pickedDisplay).controlledChildren or MateAuras.GetData(id).controlledChildren then
          wasGroup = true
  ...
  frame.GetPickedDisplay = function(self)
    if type(self.pickedDisplay) == "string" then
      return MateAuras.GetData(self.pickedDisplay)
    end
    return self.pickedDisplay
  end
