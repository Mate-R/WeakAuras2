...
local function ConstructMover(frame)
  ...
  arrowTexture:SetTexture("Interface\\Addons\\MateAuras\\Media\\Textures\\offscreen.tga")
  ...
  UpdateHighlight = function(self)
    if self.highlight then
      for _, data in ipairs(self.references) do
        local region = MateAuras.GetRegion(data.id)
...
AlignmentLines.CreateLineInformation = function(self, data, sizerPoint)
  ...
  for id, v in pairs(OptionsPrivate.displayButtons) do
    local region = MateAuras.GetRegion(v.data.id)
    if not skipIds[id]
...
AlignmentLines.CreateLines = function(self, data, sizerPoint)
  ...
  local align = (MateAurasOptionsSaved.magnetAlign and not IsShiftKeyDown())
                    or (not MateAurasOptionsSaved.magnetAlign and IsShiftKeyDown())
  if align then
...
local function ConstructMoverSizer(parent)
  local frame = CreateFrame("Frame", nil, parent, "BackdropTemplateMateAuras")
  frame:SetBackdrop({
