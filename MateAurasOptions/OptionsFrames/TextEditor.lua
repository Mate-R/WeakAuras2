...
if not MateAurasSaved.editor_tab_spaces then MateAurasSaved.editor_tab_spaces = 4 end
if not MateAurasSaved.editor_font_size then MateAurasSaved.editor_font_size = 12 end -- set default font size if missing
local color_scheme = {[0] = "|r"}
local function set_scheme()
  if not MateAurasSaved.editor_theme then
    MateAurasSaved.editor_theme = "Monokai"
  end
  local theme = editor_themes[MateAurasSaved.editor_theme]
...
local function ConstructTextEditor(frame)
  ...
  if (fontPath) then
    editor.editBox:SetFont(fontPath, MateAurasSaved.editor_font_size, "")
  end
  ...
  IndentationLib.enable(editor.editBox, color_scheme, MateAurasSaved.editor_tab_spaces)
    ...
    if level == 1 then
      for k, v in pairs(editor_themes) do
        local item = {
          ...
          isNotRadio = false,
          checked = function()
            return MateAurasSaved.editor_theme == k
          end,
          func = function()
            MateAurasSaved.editor_theme = k
            set_scheme()
      ...
      LibDD:UIDropDownMenu_AddButton(
        {
          ...
          isNotRadio = true,
          checked = function()
            return MateAurasSaved.editor_bracket_matching
          end,
          func = function()
            MateAurasSaved.editor_bracket_matching = not MateAurasSaved.editor_bracket_matching
          end
        },
      level)
      LibDD:UIDropDownMenu_AddButton(
      ...
      LibDD:UIDropDownMenu_AddButton(
        {
          text = MateAuras.newFeatureString .. L["Font Size"],
          hasArrow = true,
          notCheckable = true,
          menuList = "sizes"
        },
        ...
        LibDD:UIDropDownMenu_AddButton(
          {
            ...
            isNotRadio = false,
            checked = function()
              return MateAurasSaved.editor_tab_spaces == i
            end,
            func = function()
              MateAurasSaved.editor_tab_spaces = i
              IndentationLib.enable(editor.editBox, color_scheme, MateAurasSaved.editor_tab_spaces)
              editor.editBox:SetText(editor.editBox:GetText().."\n")
        ...
        LibDD:UIDropDownMenu_AddButton(
          {
            ...
            isNotRadio = false,
            checked = function()
              return MateAurasSaved.editor_font_size == i
            end,
            func = function()
              MateAurasSaved.editor_font_size = i
              editor.editBox:SetFont(fontPath, MateAurasSaved.editor_font_size, "")
            end
          },
        level)
  ...
  MateAurasOptionsSaved.savedSnippets = MateAurasOptionsSaved.savedSnippets or {}
  local savedSnippets = MateAurasOptionsSaved.savedSnippets
  ...
  local snippetsFrame = CreateFrame("Frame", "MateAurasSnippets", group.frame, "PortraitFrameTemplate")
  ...
  apiSearchFrame = CreateFrame("Frame", "MateAurasAPISearchFrame", group.frame, "PortraitFrameTemplate")
  ...
  -- filter line
  local filterInput = CreateFrame("EditBox", "MateAurasAPISearchFilterInput", apiSearchFrame, "SearchBoxTemplate")
  ...
  editor.editBox:HookScript(
    "OnChar",
    function(_, char)
      if not IsControlKeyDown() and MateAurasSaved.editor_bracket_matching then
        if char == "(" then
