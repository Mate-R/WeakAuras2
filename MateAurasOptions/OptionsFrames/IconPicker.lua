...
local function ConstructIconPicker(frame)
  ...
  local function iconPickerFill(subname, doSort)
    ...
    local AddButton = function(name, icon)
      local button = AceGUI:Create("MateAurasIconButton");
      button:SetName(name);
  ...
  local input = CreateFrame("EditBox", "MateAurasFilterInput", group.frame, "SearchBoxTemplate")
  ...
  local icon = AceGUI:Create("MateAurasIconButton");
  icon.frame:Disable();
  ...
  function group.Pick(self, texturePath)
    ...
    if self.groupIcon then
      ...
      MateAuras.Add(self.baseObject)
      MateAuras.ClearAndUpdateOptions(self.baseObject.id)
      MateAuras.UpdateThumbnail(self.baseObject)
    else
      for child in OptionsPrivate.Private.TraverseLeafsOrAura(self.baseObject) do
        ...
        MateAuras.UpdateThumbnail(child);
      end
    end
  ...
  function group.CancelClose()
    ...
    if group.groupIcon then
      ...
      MateAuras.Add(group.baseObject)
      MateAuras.ClearAndUpdateOptions(group.baseObject.id)
      MateAuras.UpdateThumbnail(group.baseObject)
    else
