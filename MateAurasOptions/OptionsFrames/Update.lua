...
local function Diff(ours, theirs)
  ...
  -- generates a diff which MateAuras.Update can use
  local debug = false
...
local function BuildUidMap(data, children, type)
  ...
  local function handle(data)
    ...
    then
      ...
      elseif data.anchorFrameFrame:sub(1, 9) == "MateAuras:" then
        local target = data.anchorFrameFrame:sub(10)
        if idToUid[target] then
          uidMap.map[data.uid].anchorFrameFrame = idToUid[target]
        end
      end
  ...
  uidMap.GetPhase2Data = function(self, uid, withAppliedPath, activeCategories)
    ...
    if self.map[uid].anchorFrameFrame then
      ...
      if target then
        data.anchorFrameFrame = "MateAuras:" .. target
      end
    end
  ...
  uidMap.EnsureUniqueIdOfUnmatched = function(self, uid, IncProgress)
    ...
    if not self:GetUIDMatch(uid) then
      if OptionsPrivate.Private.GetDataByUID(uid) then
        local newUid = MateAuras.GenerateUniqueID()
...
local function AddAuraList(container, uidMap, list, categories, expandText)
  local expand = AceGUI:Create("MateAurasExpand")
  local collapsed = true
  local image = collapsed and "Interface\\AddOns\\MateAuras\\Media\\Textures\\expand"
                           or "Interface\\AddOns\\MateAuras\\Media\\Textures\\collapse"
  ...
  local auraLabelContainer = AceGUI:Create("MateAurasInlineGroup")
...
local methods = {
  Open = function(self, data, children, target, linkedAuras, sender, callbackFunc)
    ...
    do
      ...
      if (highestVersion > MateAuras.InternalVersion()) then
        ...
        highestVersionWarning:SetText(L["This aura was created with a newer version of MateAuras.\nUpgrade your version of MateAuras or wait for next release before installing this aura."])
    ...
    if matchInfo ~= nil then
      if not hasChanges(matchInfo) then
        ...
        self:AddChild(AceGUI:Create("MateAurasSpacer"))
        ...
        local updateUiArea = AceGUI:Create("MateAurasInlineGroup")
    ...
    local currentBuild = floor(MateAuras.BuildInfo / 10000)
  ...
  CreateUpdateArea = function(self, area, matchInfo)
    area:AddChild(AceGUI:Create("MateAurasSpacer"))
  ...
  Import = function(self)
    if MateAuras.IsClassicEra() and C_GameRules.IsHardcoreActive() then
      StaticPopupDialogs["MateAuras_CONFIRM_IMPORT_HARDCORE"] = {
      ...
      StaticPopup_Show("MateAuras_CONFIRM_IMPORT_HARDCORE")
      return
    end
    OptionsPrivate.Private:Async({name = "import"}, function()
      self:ImportImpl()
  ...
  ImportImpl = function(self)
    ...
    if pendingPickData then
      ...
      MateAuras.PickDisplay(pendingPickData.id, pendingPickData.tabToShow)
    end
  ...
  -- Note: There's a final renaming via MateAuras.Rename at the end of the update process
  FixUpNames = function(self, uidMap, uid)
    ...
    if existingData then
      ...
      if MateAuras.GetData(uidMap:GetIdFor(uid)) then
  ...
  UpdatePhase1 = function(self, structureUidMap, uid, GetPhase1Data, phase2Order)
    ...
    MateAuras.Add(data)
    MateAuras.NewDisplayButton(data, true)
    self:IncProgress10()
  ...
  UpdatePhase2 = function(self, structureUidMap, GetPhase2Data, phase2Order, copies)
    for i = #phase2Order, 1, -1 do
      ...
      MateAuras.UpdateGroupOrders(data)
  ...
  AddProgressWidgets = function(self)
    ...
    local progress = AceGUI:Create("MateAurasProgressBar")
