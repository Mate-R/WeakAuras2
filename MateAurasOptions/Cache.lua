...
local spellCache = {}
MateAuras.spellCache = spellCache
...
function spellCache.Build()
  if not cache  then
    error("spellCache has not been loaded. Call MateAuras.spellCache.Load(...) first.")
  end
...
function spellCache.Load(data)
  ...
  local version = MateAuras.versionString
...
function spellCache.CorrectAuraName(input)
  ...
  local spellId = MateAuras.SafeToNumber(input)
  if type(input) == "string" and input:find("|", nil, true) then
    spellId = MateAuras.SafeToNumber(input:match("|Hspell:(%d+)"))
  end
  if(spellId) then
