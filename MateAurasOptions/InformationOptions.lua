...
function OptionsPrivate.GetInformationOptions(data)
  ...
  if not isTmpGroup then
    args.name = {
      ...
      set = function(info, newid)
        if data.id ~= newid and not MateAuras.GetData(newid) then
          local oldid = data.id
  ...
  if OptionsPrivate.HasWagoUrl(data.id) then
    args.ignoreWagoUpdate = {
      ...
      set = function(info, v)
          local auraData = MateAuras.GetData(data.id)
          if auraData then
            local ignoreUpdate
    ...
    for child in OptionsPrivate.Private.TraverseLeafsOrAura(data) do
      if child.information.saved then
        args["savedData." .. child.uid] = {
          ...
          width = savedDataCount > 1 and MateAuras.doubleWidth or MateAuras.normalWidth,
          order = order,
        }
        order = order + 1
      end
  ...
  if not sameDebugLog or commonDebugLog then
    args.debugLogShow = {
      ...
      func = function()
        --- @type string
        local fullMessage = L["MateAuras %s on WoW %s"]:format(MateAuras.versionString, MateAuras.BuildInfo) .. "\n\n"
        --- @type boolean
