...
local function addControlsForChange(args, order, data, conditionVariable, totalAuraCount, conditions, i, j, allProperties, usedProperties, conditionTemplates)
  ...
  args["condition" .. i .. "property" .. j] = {
    ...
    control = "MateAurasTwoColumnDropdown",
    get = function()
      local property = conditions[i].changes[j].property;
      ...
      elseif (property == "DELETE") then
        if (data.controlledChildren) then
          for id, reference in pairs(conditions[i].changes[j].references) do
            local auraData = MateAuras.GetData(id);
  ...
  elseif (propertyType == "bool" or propertyType == "number") then
    ...
    if (propertyType == "number") then
      ...
      if (properties and (properties.min or properties.softMin)) and (properties.max or properties.softMax) then
        ...
        args["condition" .. i .. "value" .. j].control = "MateAurasSpinBox"
        ...
        args["condition" .. i .. "value" .. j].validate = MateAuras.ValidateNumeric;
      end
    else
  ...
  elseif (propertyType == "string" or propertyType == "texture") then
    ...
    if propertyType == "texture" then
      args["condition" .. i .. "value" .. j].width =  MateAuras.normalWidth - 0.15
  ...
  elseif (propertyType == "icon") then
    args["condition" .. i .. "value" .. j] = {
      type = "input",
      width = MateAuras.normalWidth - 0.15,
  ...
  elseif (propertyType == "list" or propertyType == "progressSource" or propertyType == "textureLSM") then
    ...
    if propertyType == "progressSource" then
      args["condition" .. i .. "value" .. j].control = "MateAurasTwoColumnDropdown"
  ...
  elseif (propertyType == "sound") then
    ...
    args["condition" .. i .. "value" .. j .. "sound"] = {
      ...
      itemControl = "MateAurasMediaSound",
  ...
  elseif (propertyType == "chat") then
    ...
    args["condition" .. i .. "value" .. j .. "message dest_text_replacements_button"] = {
      ...
      imageHeight = 24,
      control = "MateAurasIcon",
      image = "Interface\\AddOns\\MateAuras\\Media\\Textures\\sidebar",
    }
    order = order + 1;
    ...
    args["condition" .. i .. "value" .. j .. "custom"] = {
      ...
      control = "MateAurasMultiLineEditBox",
        ...
        extraFunctions = {
          {
            ...
            func = function()
              if (data.controlledChildren) then
                ...
                OptionsPrivate.OpenTextEditor(data, multipath, nil, true, nil, nil, "https://github.com/Mate-R/MateAuras/wiki/Custom-Code-Blocks#chat-message---custom-code-1");
              else
                OptionsPrivate.OpenTextEditor(data, {"conditions", i, "changes", j, "value", "custom"}, nil, nil, nil, nil, "https://github.com/Mate-R/MateAuras/wiki/Custom-Code-Blocks#chat-message---custom-code-1");
              end
            end
          }
        }
        ...
        extraFunctions = {
          {
            ...
            func = function()
              if (data.controlledChildren) then
                ...
                OptionsPrivate.OpenTextEditor(data, multipath, true, true, nil, nil, "https://github.com/Mate-R/MateAuras/wiki/Custom-Code-Blocks#run-custom-code");
              else
                data.conditions[i].changes[j].value = data.conditions[i].changes[j].value or {};
                OptionsPrivate.OpenTextEditor(data, {"conditions", i, "changes", j, "value", "custom"}, true, nil, nil, nil, "https://github.com/Mate-R/MateAuras/wiki/Custom-Code-Blocks#run-custom-code");
              end
            end
          }
        }
...
local function addControlsForIfLine(args, order, data, conditionVariable, totalAuraCount, conditions, i, path, conditionTemplates, conditionTemplateWithoutCombinations, allProperties, parentType)
  ...
  local normalWidth = MateAuras.normalWidth - indentWidth;
  ...
  if (indentWidth > 0) then
    ...
    args["space" .. order] = {
      ...
      image = function() return "", 0, 0 end,
      order = order,
      width = MateAuras.doubleWidth * 1.5,
    }
    order = order + 1;
    ...
    args["condition" .. i .. tostring(path) .. "bullet"] = {
      ...
      name = "",
      order = order,
      image = "Interface\\Addons\\MateAuras\\Media\\Textures\\bullet" .. indentDepth,
      imageWidth = 10,
      imageHeight = 10,
    }
    order = order + 1;
      ...
      if (conditionTemplatesToUse.indexToTrigger[v] == "REMOVE") then
        if (data.controlledChildren) then
          for id, reference in pairs(conditions[i].check.references) do
            ...
            MateAuras.Add(auraData)
            MateAuras.ClearAndUpdateOptions(auraData.id)
          end
        else
          ...
          extraFunctions = {
            {
              ...
              func = function()
                if (data.controlledChildren) then
                  ...
                  OptionsPrivate.OpenTextEditor(data, multipath, nil, true, nil, nil, "https://github.com/Mate-R/MateAuras/wiki/Custom-Code-Blocks#custom-check");
                else
                  ...
                  OptionsPrivate.OpenTextEditor(data, fullPath, nil, nil, nil, nil, "https://github.com/Mate-R/MateAuras/wiki/Custom-Code-Blocks#custom-check");
                end
              end
            }
          }
...
local function addControlsForCondition(args, order, data, conditionVariable, totalAuraCount, conditions, i, conditionTemplates, conditionTemplateWithoutCombinations, allProperties)
  ...
  args["condition" .. i .. "header"] = {
    ...
    order = order,
    width = MateAuras.doubleWidth - 0.6,
    func = function()
      if data.controlledChildren then
        for id, reference in pairs(conditions[i].check.references) do
          local index = reference.conditionIndex
    ...
    image = collapsed and "Interface\\AddOns\\MateAuras\\Media\\Textures\\expand" or "Interface\\AddOns\\MateAuras\\Media\\Textures\\collapse" ,
    imageWidth = 18,
    imageHeight = 18,
    control = "MateAurasExpand",
  ...
  args["condition" .. i .. "up"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\MateAuras\\Media\\Textures\\moveup",
    imageWidth = 24,
    imageHeight = 24,
    control = "MateAurasIcon"
  };
  ...
  args["condition" .. i .. "down"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\MateAuras\\Media\\Textures\\movedown",
    imageWidth = 24,
    imageHeight = 24,
    control = "MateAurasIcon"
  };
  ...
  args["condition" .. i .. "duplicate"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\MateAuras\\Media\\Textures\\duplicate",
    imageWidth = 24,
    imageHeight = 24,
    control = "MateAurasIcon"
  };
  ...
  args["condition" .. i .. "delete"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\MateAuras\\Media\\Textures\\delete",
    imageWidth = 24,
    imageHeight = 24,
    control = "MateAurasIcon"
  };
