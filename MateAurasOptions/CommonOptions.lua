...
local function addCollapsibleHeader(options, key, input, order, isGroupTab)
  ...
  local titleWidth = MateAuras.doubleWidth - (hasAdd and 0.15 or 0) - (hasDelete and 0.15 or 0)  - (hasUp and 0.15 or 0)
  ...
  if not withoutheader then
    options[key .. "collapseButton"] = {
      ...
      image = function()
        if notcollapsable then
          return "Interface\\AddOns\\MateAuras\\Media\\Textures\\bullet1", 18, 18
        else
          return isCollapsed() and "Interface\\AddOns\\MateAuras\\Media\\Textures\\expand"
    ...
    if hasApplyTemplate then
      options[key .. "applyTemplate"] = {
        ...
        order = order + 0.7,
        width = 0.15,
        image = "Interface\\AddOns\\MateAuras\\Media\\Textures\\template",
        imageWidth = 24,
        imageHeight = 24,
        control = "MateAurasIcon",
        hidden = hiddenFunc
...
local function ProgressOptions(data)
  ...
  options.adjustedMin = {
    type = "input",
    validate = MateAuras.ValidateNumericOrPercent,
    width = MateAuras.normalWidth,
    order = order + 2,
    name = L["Minimum"],
...
local function PositionOptions(id, data, _, hideWidthHeight, disableSelfPoint, group)
  ...
  local positionOptions = {
    ...
    selfPoint = {
      ...
      control = "MateAurasAnchorButtons",
    },
    anchorPoint = {
      type = "select",
      width = MateAuras.normalWidth,
  ...
  OptionsPrivate.commonOptions.AddCodeOption(positionOptions, data, L["Custom Anchor"], "custom_anchor",
                      "https://github.com/Mate-R/MateAuras/wiki/Custom-Code-Blocks#custom-anchor-function",
...
local function AddCommonTriggerOptions(options, data, triggernum, doubleWidth)
  ...
  options.type = {
    type = "select",
    width = doubleWidth and MateAuras.doubleWidth or MateAuras.normalWidth,
    name = L["Type"],
