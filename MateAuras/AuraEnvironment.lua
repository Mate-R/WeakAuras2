...
MateAuras.WA_Utf8Sub = WA_Utf8Sub
...
local WA_ClassColorName = function(unit, maxlen)
  if unit and UnitExists(unit) then
    local name = MateAuras.UnitName(unit)
...
MateAuras.WA_ClassColorName = WA_ClassColorName

MateAuras.PadString = function(input, padMode, padLength)
  input = tostring(input)
  if type(input) ~= "string" then
    return input
  end
...
MateAuras.GetUnitFrame = LGF.GetUnitFrame
MateAuras.GetUnitNameplate =  function(unit)
...
local blockedTables = {
  ...
  ChatFrame1 = true,
  MateAurasSaved = true,
  MateAurasOptions = true,
  MateAurasOptionsSaved = true,
  ItemRackUser = true,
  ItemRackEvents = true
}
...
-- Stack of of aura environments/uids, allows use of recursive aura activations through calls to MateAuras.ScanEvents().
...
function Private.ActivateAuraEnvironment(id, cloneId, state, states, onlyConfig)
  local data = id and MateAuras.GetData(id)
  if not data then
    ...
    if environment_initialized[id] == 2 or (onlyConfig and environment_initialized[id] == 1) then
      local region = MateAuras.GetRegion(id, cloneId)
    ...
    elseif onlyConfig then
      ...
      -- Either this aura environment has not yet been initialized, or it was reset via an edit in MateAurasOptions
...
local FakeMateAurasMixin = {
  blockedFunctions = {
    ...
    ShowOptions = true,
    -- Note these shouldn't exist in the MateAuras namespace, but moving them takes a bit of effort,
  ...
  blockedTables = {
    ...
    -- checked for MateAuras.loaded in there
    loaded = true
  },
  override = {
    ...
    GetData = function(id)
      ...
      if getDataCallCounts[currentId] > 99 then
        Private.AuraWarnings.UpdateWarning(current_uid, "FakeMateAurasGetData", "warning",
    ...
    clones = MakeDeprecated(Private.clones, "clones",
                L["Using MateAuras.clones is deprecated. Use MateAuras.GetRegion(id, cloneId) instead."]),
    regions = MakeDeprecated(Private.regions, "regions",
                L["Using MateAuras.regions is deprecated. Use MateAuras.GetRegion(id) instead."]),
  ...
  blocked = blocked,
  setBlocked = function()
    Private.AuraWarnings.UpdateWarning(current_uid, "FakeMateAurasSet", "error",
                  L["Writing to the MateAuras table is not allowed."], true)
  end
}

local FakeMateAuras = MakeReadOnly(MateAuras, FakeMateAurasMixin)
...
local overridden = {
  ...
  ActionButton_ShowOverlayGlow = MateAuras.ShowOverlayGlow,
  ActionButton_HideOverlayGlow = MateAuras.HideOverlayGlow,
  MateAuras = FakeMateAuras,
  WeakAuras = FakeMateAuras,
}
...
local exec_env_custom = setmetatable(CopyTable(mixins),
{
  ...
  __newindex = function(table, key, value)
    if key == "aura_env" then
      ...
      -- please try harder to not hallucinate about MateAuras, thanks
...
function MateAuras.LoadFunction(string, id)
