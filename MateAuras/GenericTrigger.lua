...
#####################################################
# Helper functions mainly for the MateAuras Options #
#####################################################
...
function MateAuras.UnitExistsFixed(unit, smart)
...
function MateAuras.split(input)
  input = input or "";
  local ret = {};
...
function MateAuras.ScanEventsByID(event, id, ...)
...
function MateAuras.ScanEvents(event, arg1, arg2, ...)
      ...
      for id, triggers in pairs(event_list) do
        ...
        for triggernum, data in pairs(triggers) do
          ...
          then
            ...
            if delay == 0 then
              local allStates = MateAuras.GetTriggerStateForTrigger(id, triggernum);
...
function MateAuras.ScanUnitEvents(event, unit, ...)
...
function MateAuras.ScanEventsInternal(event_list, event, arg1, arg2, ... )
...
function Private.ScanEventsWatchedTrigger(id, watchedTriggernums)
  ...
  for _, watchedTrigger in ipairs(watchedTriggernums) do
    if watched_trigger_events[id] and watched_trigger_events[id][watchedTrigger] then
      local updatedTriggerStates = MateAuras.GetTriggerStateForTrigger(id, watchedTrigger)
      for observerTrigger in pairs(watched_trigger_events[id][watchedTrigger]) do
        ...
        local allstates = MateAuras.GetTriggerStateForTrigger(id, observerTrigger)
...
function HandleEvent(frame, event, arg1, arg2, ...)
  ...
  if not(MateAuras.IsPaused()) then
...
---@class MateAurasGenericTriggerFrame: FrameScriptObject
local frame = CreateFrame("Frame");
frame.unitFrames = {};
Private.frames["MateAuras Generic Trigger Frame"] = frame;
      ...
      if(Private.category_event_prototype[triggerType] or triggerType == "custom") then
        ...
        if(Private.category_event_prototype[triggerType]) then
          if not(trigger.event) then
            error("Improper arguments to MateAuras.Add - trigger type is \"event\" but event is not defined");
          elseif not(event_prototypes[trigger.event]) then
            if(event_prototypes["Conditions"]) then
              trigger.event = "Conditions";
            else
              error("Improper arguments to MateAuras.Add - no event prototype can be found for event type \""..trigger.event.."\" and default prototype reset failed.");
            end
          else
            ...
            elseif prototype.timedrequired then
              ...
              MateAuras.prettyPrint("Invalid Prototype found: " .. prototype.name)
            end
          ...
          triggerFunc = MateAuras.LoadFunction("return "..(trigger.custom or ""), data.id);
          if (trigger.custom_type == "stateupdate") then
            tsuConditionVariables = MateAuras.LoadFunction("return function() return \n" .. (trigger.customVariables or "") .. "\n end", data.id);
          ...
          if(trigger.custom_type == "status" or trigger.custom_type == "event" and trigger.custom_hide == "custom") then
            untriggerFunc = MateAuras.LoadFunction("return "..(untrigger.custom or ""), data.id);
          ...
          if(trigger.custom_type ~= "stateupdate" and trigger.customDuration and trigger.customDuration ~= "") then
            durationFunc = MateAuras.LoadFunction("return "..trigger.customDuration, data.id);
          end
          if(trigger.custom_type ~= "stateupdate") then
            overlayFuncs = {};
            for i = 1, 7 do
          ...
          if((trigger.custom_type == "status" or trigger.custom_type == "stateupdate") and trigger.check == "update") then
            ...
            local rawEvents = MateAuras.split(trigger.events);
...
do
  ...
  local function CheckGCD()
    ...
    if(event and not MateAuras.IsPaused()) then
  ...
  local function HandleSpell(self, id, startTime, duration, modRate, paused)
    ...
    if C_Secrets.ShouldSpellCooldownBeSecret(id) then
      local isReady = MateAuras.IsSpellReady(id)
    ...
    if duration > 0 then
      if (startTime == gcdStart and duration == gcdDuration)
          or (MateAuras.IsClassicOrWrath() and duration == shootDuration and startTime == shootStart)
      then
  ...
  local SpellDetails = {
    ...
    AddEffectiveSpellId = function(self, effectiveSpellId, userSpellId)
      ...
      spellDetail.known = MateAuras.IsSpellKnownIncludingPet(effectiveSpellId)
      ...
      local charges, maxCharges, startTime, duration, unifiedCooldownBecauseRune,
            ...
            = MateAuras.GetSpellCooldownUnified(effectiveSpellId, GetRuneDuration());
    ...
    CheckSpellKnown = function(self)
      ...
      for effectiveSpellId, spellDetailsData in pairs(self.data) do
        local known = MateAuras.IsSpellKnownIncludingPet(effectiveSpellId)
    ...
    CheckSpellCooldown = function(self, effectiveSpellId, runeDuration)
      local charges, maxCharges, startTime, duration, unifiedCooldownBecauseRune,
        startTimeCooldown, durationCooldown, cooldownBecauseRune, startTimeCharges, durationCharges,
        spellCount, unifiedModRate, modRate, modRateCharges, paused
        = MateAuras.GetSpellCooldownUnified(effectiveSpellId, runeDuration);
    ...
    WatchSpellCooldown = function(self, userSpellId, ignoreRunes, useExact, followoverride)
      ...
      if ignoreRunes and MateAuras.IsWrathOrCataOrMistsOrRetail() then
        for i = 1, 6 do
  ...
  function MateAuras.GetRuneCooldown(id)
  ...
  local essenceCache = {{},{},{},{},{},{}}
  ---@private
  function MateAuras.InitEssenceCooldown()
  ...
  function MateAuras.GetEssenceCooldown(essence)
  ...
  function MateAuras.GetSpellLossOfControlCooldown(identifier)
    if MateAuras.IsTWW() then
  ...
  function MateAuras.GetSpellCooldown(id, ignoreRuneCD, showgcd, ignoreSpellKnown, track)
  ...
  function MateAuras.GetSpellCharges(id, ignoreSpellKnown)
  ...
  ---@return number gcdCooldown
  function MateAuras.GetItemCooldown(id, showgcd)
  ...
  ---@return number duration
  function MateAuras.gcdDuration()
    return gcdDuration or 0;
  end

  ---@return string name
  function MateAuras.GcdSpellName()
    return gcdSpellName;
  end
  ...
  ---@return boolean enabled
  ---@return number gcdCooldown
  function MateAuras.GetItemSlotCooldown(id, showgcd)
    local startTime, duration, enabled, gcdCooldown;
  ...
  function Private.CheckRuneCooldown()
    ...
    for id, _ in pairs(runes) do
      ...
      if(duration > 0 and duration ~= MateAuras.gcdDuration()) then
        -- On non-GCD cooldown
        local endTime = startTime + duration;
  ...
  function MateAuras.GetSpellCooldownUnified(id, runeDuration)
  ...
  function MateAuras.IsSpellReady(id)
  ...
  function Private.CheckItemCooldowns()
    for id, _ in pairs(items) do
      ...
      if (itemCdEnabledChanged and not MateAuras.IsPaused()) then
        Private.ScanEventsByID("ITEM_COOLDOWN_CHANGED", id);
      end
    end
  end
  ...
  ---@private
  function MateAuras.WatchRuneCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
  ...
  ---@private
  function MateAuras.WatchSpellCooldown(id, ignoreRunes, useExact, followoverride)
    SpellDetails:WatchSpellCooldown(id, ignoreRunes, useExact, followoverride)
  end

  ---@private
  function MateAuras.WatchItemCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
    ...
    if not(items[id]) then
      ...
      itemCdEnabled[id] = enabled;
      if(duration and duration > 0 and duration > 1.5 and duration ~= MateAuras.gcdDuration()) then
        local time = GetTime();
        local endTime = startTime + duration;
        itemCdDurs[id] = duration;
        itemCdExps[id] = endTime;
  ...
  ---@private
  function MateAuras.WatchItemSlotCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
...
do
  ...
  ---@private
  function MateAuras.WatchSpellActivation(id)
    if (not id) then
      return;
    end
  ...
  function MateAuras.SpellActivationActive(id)
...
function MateAuras.GetPlayerReaction(unit)
...
---@param unit UnitToken
function MateAuras.WatchUnitChange(unit)
  unit = string.lower(unit)
  if not watchUnitChange then
    ...
    local function unitUpdate(unitA, eventsToSend)
      ...
      local newGUID = MateAuras.UnitExistsFixed(unitA) and UnitGUID(unitA)
    ...
    local function reactionUpdate(unit, eventsToSend)
      ...
      local newReaction = MateAuras.GetPlayerReaction(unit)
    ...
    local function reactionInit(unit)
      watchUnitChange.nameplateFaction[unit] = MateAuras.GetPlayerReaction(unit)
    end
    ...
    if MateAuras.IsClassicOrWrath() then
      function roleUpdate(unit, eventsToSend)
        ...
        local newRaidRole = MateAuras.UnitRaidRole(unit)
    ...
    local handleEvent = {
      ...
      UNIT_PET = function(unit, eventsToSend)
        local pet = MateAuras.unitToPetUnit[unit]
  ...
  watchUnitChange.unitIdToGUID[unit] = MateAuras.UnitExistsFixed(unit) and UnitGUID(unit)
...
function MateAuras.GetEquipmentSetInfo(itemSetName, partial)
...
function Private.WatchStagger()
  if not staggerWatchFrame then
    ...
    Private.frames["MateAuras Stagger Frame"] = staggerWatchFrame
...
if MateAuras.IsClassicOrWrath() then
  ...
  function MateAuras.WatchForQueuedSpell()
  ...
  function MateAuras.GetQueuedSpell()
...
---@return number? cost
function MateAuras.GetSpellCost(powerTypeToCheck)
  local spellID = select(9, MateAuras.UnitCastingInfo("player"))
  if MateAuras.IsClassicOrWrath() and not spellID then
    spellID = MateAuras.GetQueuedSpell()
  end
  if spellID then
...
-- Weapon Enchants
do
  local isCata = MateAuras.IsCataClassic()
  ...
  function MateAuras.TenchInit()
    if not(tenchFrame) then
      ...
      if MateAuras.IsRetail() then
        ...
        getTenchName = function(id)
          tenchTip = MateAuras.GetHiddenTooltip();
...
do
  ...
  ---@private
  function MateAuras.WatchForPetDeath()
...
do
  ...
  ---@private
  function MateAuras.WatchForCastLatency()
    if not castLatencyFrame then
      ...
      -- triggered from cacheEmpoweredFrame after updating cache use by MateAuras.UnitChannelInfo
  ...
  function MateAuras.GetCastLatency()
...
do
  ...
  ---@private
  function MateAuras.WatchForNameplateTargetChange()
...
do
  ...
  ---@private
  function MateAuras.WatchForPlayerMoving()
...
if C_AssistedCombat and C_AssistedCombat.GetNextCastSpell then
  ...
  ---@private
  function MateAuras.WatchForAssistedCombatNextCast()
...
---@private
function MateAuras.RegisterItemCountWatch()
...
function MateAuras.GetUniqueCloneId()
...
local commonConditions = {
  ...
  itemInRange = {
    display = MateAuras.newFeatureString .. L["Item in Range"],
    hidden = true,
    type = "bool",
    test = function(state, needle)
        ...
        if (enable) then
          ...
          if (result[v.name].type == "select" or result[v.name].type == "unit") then
            if (v.conditionValues) then
              result[v.name].values = Private[v.conditionValues] or MateAuras[v.conditionValues];
            else
              if type(v.values) == "function" then
                result[v.name].values = v.values()
              else
                result[v.name].values = Private[v.values] or MateAuras[v.values];
              end
            end
          end
...
do
  ...
  if class == "DEMONHUNTER"
    ...
    or class == "EVOKER"
  then
    function MateAuras.CalculatedGcdDuration()
...
MateAuras.CheckForItemBonusId = function(ids)
...
MateAuras.GetBonusIdInfo = function(ids, specificSlot)
...
MateAuras.CheckForItemEquipped = function(itemId, specificSlot)
...
Private.ExecEnv.GetCurrencyAccountInfo = function(currencyId)
  ...
  if MateAuras.IsRetail() and currencyInfo and currencyInfo.isAccountTransferable then
  ...
  -- WORKAROUND https://github.com/Mate-R/MateAuras/issues/5106
  if currencyInfo and MateAuras.IsCataClassic() then
...
-- So for now make it simply a member of MateAuras
MateAuras.genericTriggerTypes = types

MateAuras.RegisterTriggerSystem(types, GenericTrigger);
