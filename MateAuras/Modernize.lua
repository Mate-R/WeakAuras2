...
function Private.Modernize(data, oldSnapshot)
  if not data.internalVersion or data.internalVersion < 2 then
    MateAuras.prettyPrint(string.format("Data for '%s' is too old, can't modernize.", data.id))
  ...
  -- Version 10 is skipped, due to a bad migration script (see https://github.com/Mate-R/MateAuras/pull/1091)
    ...
    for triggerId, triggerData in ipairs(data.triggers) do
      if triggerData.trigger.type == "status" or triggerData.trigger.type == "event" then
        ...
        if newType then
          ...
          MateAuras.prettyPrint("Unknown trigger type found in, please report: ", data.id, triggerData.trigger.event)
        end
      end
    end
  end
  ...
  if data.internalVersion == 49 then
    ...
    if broken then
      MateAuras.prettyPrint(L["Trying to repair broken conditions in %s likely caused by a MateAuras bug."]:format(data.id))
    end
  end
  ...
  if data.internalVersion < 75 then
    -- this commit from nov 2019 https://github.com/Mate-R/MateAuras/commit/6d8f11c17422aeffdb82a0aa05181edfdd137896
    ...
    -- this commit https://github.com/Mate-R/MateAuras/commit/dbcb70b1e4df262af82f63620b3b0d80741e6df2
    ...
    for _, when in ipairs({ "start", "finish" }) do
      if data.actions[when] then
        if type(data.actions[when].sound) == "string" then
          data.actions[when].sound = data.actions[when].sound:gsub("^Interface\\Add[oO]ns\\WeakAuras\\", "Interface\\AddOns\\MateAuras\\")
        end
      end
    end
      ...
      for conditionIndex, condition in ipairs(data.conditions) do
        for changeIndex, change in ipairs(condition.changes) do
          if change.property == "sound" and type(change.value.sound) == "string" then
            change.value.sound = change.value.sound:gsub("^Interface\\Add[oO]ns\\WeakAuras\\", "Interface\\AddOns\\MateAuras\\")
          end
        end
      end
    end
  ...
  if data.internalVersion < 89 then
    for _, when in ipairs({ "start", "finish" }) do
      if data.actions[when] then
        if type(data.actions[when].sound) == "string" then
          data.actions[when].sound = data.actions[when].sound:gsub("^Interface\\Add[oO]ns\\Mate2Auras\\", "Interface\\AddOns\\MateAuras\\")
          data.actions[when].sound = data.actions[when].sound:gsub("^Interface\\Add[oO]ns\\WeakAuras\\", "Interface\\AddOns\\MateAuras\\")
        end
      end
    end
    ...
    if data.conditions then
      for conditionIndex, condition in ipairs(data.conditions) do
        for changeIndex, change in ipairs(condition.changes) do
          if change.property == "sound" and type(change.value.sound) == "string" then
            change.value.sound = change.value.sound:gsub("^Interface\\Add[oO]ns\\Mate2Auras\\", "Interface\\AddOns\\MateAuras\\")
            change.value.sound = change.value.sound:gsub("^Interface\\Add[oO]ns\\WeakAuras\\", "Interface\\AddOns\\MateAuras\\")
          end
        end
      end
  ...
  data.internalVersion = max(data.internalVersion or 0, MateAuras.InternalVersion())
end
