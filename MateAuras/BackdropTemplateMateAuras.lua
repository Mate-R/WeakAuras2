BackdropTemplateMateAurasMixin = { };
...
function BackdropTemplateMateAurasMixin:OnBackdropLoaded()
...
function BackdropTemplateMateAurasMixin:OnBackdropSizeChanged()
	if self.backdropInfo then
		self:SetupTextureCoordinates();
	end
end

function BackdropTemplateMateAurasMixin:GetEdgeSize()
...
function BackdropTemplateMateAurasMixin:SetupTextureCoordinates()
...
function BackdropTemplateMateAurasMixin:SetupPieceVisuals(piece, setupInfo, pieceLayout)
...
function BackdropTemplateMateAurasMixin:SetBorderBlendMode(blendMode)
...
function BackdropTemplateMateAurasMixin:HasBackdropInfo(backdropInfo)
	return self.backdropInfo == backdropInfo;
end

function BackdropTemplateMateAurasMixin:ClearBackdrop()
	if self.backdropInfo then
		for pieceName in pairs(textureUVs) do
			local region = self[pieceName];
			if region then
...
function BackdropTemplateMateAurasMixin:ApplyBackdrop()
	...
	local layout = {
		...
		setupPieceVisualsFunction = BackdropTemplateMateAurasMixin.SetupPieceVisuals,
	};
...
function BackdropTemplateMateAurasMixin:SetBackdrop(backdropInfo)
...
function BackdropTemplateMateAurasMixin:GetBackdrop()
	if self.backdropInfo then
...
function BackdropTemplateMateAurasMixin:GetBackdropColor()
	if not self.backdropInfo then
		return;
	end
	if self.Center then
...
function BackdropTemplateMateAurasMixin:SetBackdropColor(r, g, b, a)
	if not self.backdropInfo then
...
function BackdropTemplateMateAurasMixin:GetBackdropBorderColor()
	if not self.backdropInfo then
		return
	end
...
function BackdropTemplateMateAurasMixin:SetBackdropBorderColor(r, g, b, a)
	if not self.backdropInfo then
		-- Ideally this would throw an error here but the old API just failed silently
		return;
	end
...
function BackdropTemplateMateAurasMixin:ForEachPiece(callback)
	for pieceName in pairs(textureUVs) do
		local region = self[pieceName];
		if region then
