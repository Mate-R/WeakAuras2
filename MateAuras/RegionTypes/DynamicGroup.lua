...
local function create(parent)
  ...
  region.sortStates = {}
  region.growStates = {}
  local background = CreateFrame("Frame", nil, region, "BackdropTemplateMateAuras")
...
function MateAuras.GetPolarCoordinates(x, y, originX, originY)
...
function MateAuras.InvertSort(sortFunc)
...
function MateAuras.SortNilLast(a, b)
...
local sortNilFirst = MateAuras.InvertSort(MateAuras.SortNilLast)
function MateAuras.SortNilFirst(a, b)
  if a == nil and b == nil then
...
function MateAuras.SortGreaterLast(a, b)
...
MateAuras.SortGreaterFirst = MateAuras.InvertSort(MateAuras.SortGreaterLast)

function MateAuras.SortRegionData(path, sortFunc)
  ...
  if type(sortFunc) ~= "function" then
    ...
    sortFunc = MateAuras.SortGreaterLast
  end
  return function(a, b)
...
function MateAuras.SortAscending(path)
  return MateAuras.SortRegionData(path, MateAuras.ComposeSorts(MateAuras.SortNilFirst, MateAuras.SortGreaterLast))
end

function MateAuras.SortDescending(path)
  return MateAuras.InvertSort(MateAuras.SortAscending(path))
end

function MateAuras.ComposeSorts(...)
...
local sorters = {
  none = function(data)
    return MateAuras.ComposeSorts(
      MateAuras.SortAscending({"dataIndex"}),
      MateAuras.SortAscending({"region", "state", "index"})
    ), { index = true }
  end,
  hybrid = function(data)
    ...
    if hybridSortAscending then
      sortExpirationTime = MateAuras.SortAscending({"region", "state", "expirationTime"})
    else
      sortExpirationTime = MateAuras.SortDescending({"region", "state", "expirationTime"})
    end
    return MateAuras.ComposeSorts(
      sortHybridStatus,
      sortExpirationTime,
  ...
  ascending = function(data)
    return MateAuras.ComposeSorts(
      MateAuras.SortAscending({"region", "state", "expirationTime"}),
      MateAuras.SortAscending({"dataIndex"})
    ), {expirationTime = true}
  end,
  descending = function(data)
    return MateAuras.ComposeSorts(
      MateAuras.SortDescending({"region", "state", "expirationTime"}),
      MateAuras.SortAscending({"dataIndex"})
    ), {expirationTime = true}
  end,
  custom = function(data)
    local sortStr = data.customSort or ""
    local sortFunc = MateAuras.LoadFunction("return " .. sortStr, data.id) or noop
    local sortOn = nil
    local events = MateAuras.split(data.sortOn or "")
    if #events > 0 then
      sortOn = {}
...
MateAuras.SortFunctions = sorters
...
local anchorers = {
  ["NAMEPLATE"] = function(data)
    return function(frames, activeRegions)
      for _, regionData in ipairs(activeRegions) do
        ...
        local found
        if unit then
          local frame = MateAuras.GetUnitNameplate(unit)
          if frame then
        ...
        if not found and MateAuras.IsOptionsOpen() and regionData.region.state then
  ...
  ["CUSTOM"] = function(data)
    ...
    local anchorFunc = MateAuras.LoadFunction("return " .. anchorStr, data.id) or noop

    local anchorOn = nil
    local events = MateAuras.split(data.anchorOn or "")
    if #events > 0 then
      anchorOn = {}
      for _, event in ipairs(events) do
        anchorOn[event] = true
...
local growers = {
  ...
  CUSTOM = function(data)
    ...
    local growFunc = MateAuras.LoadFunction("return " .. growStr, data.id) or noop
    local growOn = nil
    local events = MateAuras.split(data.growOn or "")
    if #events > 0 then
      growOn = {}
      for _, event in ipairs(events) do
        growOn[event] = true
...
MateAuras.GrowFunctions = growers
...
local function modify(parent, region, data)
  ...
  function region:IsSuspended()
    return not MateAuras.IsLoginFinished() or self.suspended > 0
  end
  ...
  function region:ReloadControlledChildren()
    ...
    if not self:IsSuspended() then
      ...
      for dataIndex, childID in ipairs(data.controlledChildren) do
        local childRegion, childData = MateAuras.GetRegion(childID), MateAuras.GetData(childID)
  ...
  function region:AddChild(childID, cloneID)
    ...
    local childRegion = MateAuras.GetRegion(childID, cloneID)
  ...
  function region:DoPositionChildrenPerFrame(frame, positions, handledRegionData)
    for regionData, pos in pairs(positions) do
      ...
      controlPoint:SetShown(show and frame ~= MateAuras.HiddenFrames)
      ...
      if animate then
        ...
        if show and (abs(xDelta) > 0.01 or abs(yDelta) > 0.01) then
          ...
          if data.grow == "CIRCLE" or data.grow == "COUNTERCIRCLE" then
            ...
            local radius1, previousAngle = MateAuras.GetPolarCoordinates(xPrev, yPrev, originX, originY)
            local radius2, newAngle = MateAuras.GetPolarCoordinates(x, y, originX, originY)
  ...
  function region:Resize()
    ...
    if not self:IsSuspended() then
      ...
      if isRestricted and not MateAuras.IsOptionsOpen() then
