...
local function create(parent, data)
  ...
  --The reason is so that MateAuras cooldown frames can interact properly with OmniCC
  -- (i.e., put on its ignore list for timer overlays)
  local id = data.id;
  local frameId = id:lower():gsub(" ", "_");
  if(_G["MateAurasCooldown"..frameId]) then
    local baseFrameId = frameId;
    local num = 2;
    while(_G["MateAurasCooldown"..frameId]) do
  ...
  local cooldown = CreateFrame("Cooldown", "MateAurasCooldown"..frameId, region, "CooldownFrameTemplate");
  ...
  then
    ElvUI[1]:RegisterCooldown(cooldown, "MateAuras");
  end
...
local function modify(parent, region, data)
  ...
  if MSQ then
    ...
    if region.masqueId ~= masqueId then
      ...
      region.MSQGroup = MSQ:Group("MateAuras", region.masqueId, data.uid);
