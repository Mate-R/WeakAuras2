...
local function CreateTestForCondition(data, input, allConditionsTemplate, usedStates)
  ...
  if (trigger and variable) then
    ...
    elseif (cType == "customcheck") then
      if value then
        local customCheck = MateAuras.LoadFunction("return " .. value, data.id)
        if customCheck then
          Private.ExecEnv.conditionHelpers[uid] = Private.ExecEnv.conditionHelpers[uid] or {}
          Private.ExecEnv.conditionHelpers[uid].customTestFunctions
            = Private.ExecEnv.conditionHelpers[uid].customTestFunctions or {}
    ...
    elseif (cType == "range" and value and op and input.type and input.op_range and input.range) then
        ...
        fn = [[
          return function()
            ...
            for unit in WA_IterateGroupMembers() do
              if not Private.ExecEnv.UnitIsUnit(unit, "player") and MateAuras.CheckRange(unit, range, op) then
                found = found + 1
              end
            end
            return found %s %d
        ...
        fn = [[
          return function()
            ...
            for i = 1, 40 do
              ...
              if UnitExists(unit) and UnitCanAttack("player", unit) and MateAuras.CheckRange(unit, range, op) then
                found = found + 1
              end
            end
            return found %s %d
      ...
      if fn then
        local customCheck = MateAuras.LoadFunction(fn, data.id)
        if customCheck then
          Private.ExecEnv.conditionHelpers[uid] = Private.ExecEnv.conditionHelpers[uid] or {}
          Private.ExecEnv.conditionHelpers[uid].customTestFunctions
            = Private.ExecEnv.conditionHelpers[uid].customTestFunctions or {}
      ...
      if (condition.changes) then
        for changeIndex, change in ipairs(condition.changes) do
          if ( (change.property == "chat" or change.property == "customcode") and type(change.value) == "table" and change.value.custom) then
            ...
            local customFunc = MateAuras.LoadFunction(prefix .. custom .. suffix, data.id);
            if (customFunc) then
...
local globalConditions =
{
  ...
  ["rangecheck"] = {
    display = MateAuras.newFeatureString .. L["Range Check"],
    type = "range",
...
local function ConstructConditionFunction(data)
  ...
  table.insert(ret, "  local activatedConditions = MateAuras.GetActiveConditions(id, cloneId)\n")
...
local function runDynamicConditionFunctions(funcs)
  for uid in pairs(funcs) do
    ...
    if (Private.IsAuraActive(uid) and checkConditions[uid]) then
      local activeStates = MateAuras.GetActiveStates(id)
