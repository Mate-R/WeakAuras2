...
local function RunAnimation(key, anim, elapsed, time)
  ...
  if(anim.translateFunc) then
    local errorHandler = MateAuras.IsOptionsOpen() and noopErrorHandler or Private.GetErrorHandlerUid(anim.auraUID, L["Slide Animation"])
  ...
  if(anim.alphaFunc) then
    local errorHandler = MateAuras.IsOptionsOpen() and noopErrorHandler or Private.GetErrorHandlerUid(anim.auraUID, L["Fade Animation"])
  ...
  if(anim.scaleFunc) then
    local errorHandler = MateAuras.IsOptionsOpen() and noopErrorHandler
...
Private.frames["MateAuras Animation Frame"] = frame
...
function Private.Animate(namespace, uid, type, anim, region, inverse, onFinished, loop, cloneId)
  ...
  if(valid) then
    ...
    if(animations[key]) then
      ...
      if issecretvalue(startWidth) then startWidth = region.width or 0 end
      if issecretvalue(startHeight) then startHeight = region.height or 0 end
      anim.rotate = anim.rotate or 0;
      anim.colorR = anim.colorR or 1;
      anim.colorG = anim.colorG or 1;
      anim.colorB = anim.colorB or 1;
    ...
    if(anim.use_translate) then
      ...
      if (anim.translateFunc) then
        translateFunc = MateAuras.LoadFunction("return " .. anim.translateFunc, uid);
      else
    ...
    if(anim.use_alpha) then
      ...
      if (anim.alphaFunc) then
        alphaFunc = MateAuras.LoadFunction("return " .. anim.alphaFunc, uid);
      else
    ...
    if(anim.use_scale) then
      ...
      if (anim.scaleFunc) then
        scaleFunc = MateAuras.LoadFunction("return " .. anim.scaleFunc, uid);
      else
        region:Scale(1, 1);
      end
    ...
    if(anim.use_rotate) then
      ...
      if (anim.rotateFunc) then
        rotateFunc = MateAuras.LoadFunction("return " .. anim.rotateFunc, uid);
      else
    ...
    if(anim.use_color) then
      ...
      if (anim.colorFunc) then
        colorFunc = MateAuras.LoadFunction("return " .. anim.colorFunc, uid);
      else
