...
function MateAuras.IsSpellInRange(spellId, unit)
  return SpellRange.IsSpellInRange(spellId, unit)
end

local LibRangeCheck = LibStub("LibRangeCheck-3.0")

function MateAuras.GetRange(unit, checkVisible)
  return LibRangeCheck:GetRange(unit, checkVisible);
end

function MateAuras.CheckRange(unit, range, operator)
...
function MateAuras.UnitDetailedThreatSituation(unit1, unit2)
...
MateAuras.UnitCastingInfo = UnitCastingInfo

if MateAuras.IsRetail() then
  local cacheEmpowered = {}
  MateAuras.UnitChannelInfo = function(unit)
  ...
  MateAuras.UnitChannelInfo = UnitChannelInfo
end

local constants = {
  nameRealmFilterDesc = L[" Filter formats: 'Name', 'Name-Realm', '-Realm'. \n\nSupports multiple entries, separated by commas\nCan use \\ to escape -."],
  instanceFilterDeprecated = L["This filter has been moved to the Location trigger. Change your aura to use the new Location trigger or join the MateAuras Discord server for help."],
...
if MateAuras.IsClassicOrWrathOrCataOrMists() then
  ...
  ---@return string? role
  MateAuras.UnitRaidRole = function(unit)
...
function MateAuras.SpellSchool(school)
  return Private.combatlog_spell_school_types[school] or ""
end

---@param flag integer
---@return integer index
function MateAuras.RaidFlagToIndex(flag)
...
---@return number b
---@return number a
function MateAuras.GetHSVTransition(perc, r1, g1, b1, a1, r2, g2, b2, a2)
...
straightHSV = [[
function(progress, r1, g1, b1, a1, r2, g2, b2, a2)
    return MateAuras.GetHSVTransition(progress, r1, g1, b1, a1, r2, g2, b2, a2)
end
]],
...
function(progress, r1, g1, b1, a1, r2, g2, b2, a2)
    local angle = (progress * 2 * math.pi) - (math.pi / 2)
    local newProgress = ((math.sin(angle) + 1)/2);
    return MateAuras.GetHSVTransition(newProgress, r1, g1, b1, a1, r2, g2, b2, a2)
end
]],
...
MateAuras.class_ids = {}
MateAuras.classes_sorted = {}
for classID = 1, 20 do -- GetNumClasses not supported by wow classic
  local classInfo = C_CreatureInfo.GetClassInfo(classID)
  if classInfo then
    MateAuras.class_ids[classInfo.classFile] = classInfo.classID
    tinsert(MateAuras.classes_sorted, classInfo.classFile)
  end
end
table.sort(MateAuras.classes_sorted)

if MateAuras.IsClassicOrWrathOrCata() then
  ---@param index integer
  ---@param extraOption boolean?
  ---@return boolean? hasTalent
  function MateAuras.CheckTalentByIndex(index, extraOption)
...
if MateAuras.IsRetail() then
  ...
  Private.frames["MateAuras talentCheckFrame"] = talentCheckFrame
  ...
  ---@return number? rank
  function MateAuras.GetTalentById(talentId)
  ...
  ---@param talentId integer
  ---@return boolean hasTalent
  function MateAuras.CheckTalentId(talentId)
...
---@return boolean hasTalent
---@return number? spellid
function MateAuras.CheckPvpTalentBySpellId(spellId)
...
function MateAuras.CheckNumericIds(loadids, currentId)
...
function MateAuras.ValidateNumeric(info, val)
...
function MateAuras.ValidateTime(info, val)
...
function MateAuras.TimeToSeconds(val)
...
---@param val string
---@return boolean result
function MateAuras.ValidateNumericOrPercent(info, val)
  if val ~= nil and val ~= "" then
...
---@return boolean result
function MateAuras.IsPlayerSpellOrOverridesAndBaseIsPlayerSpell(spell)
...
---@private
function MateAuras.IsSpellKnownForLoad(spell, exact)
  if spell == 0 or spell >= 2^31 then return false end
  ...
  if (spellName) then
    ...
    if otherSpell and otherSpell ~= spell then
      return MateAuras.IsSpellKnownForLoad(otherSpell)
    end
  end
end
...
---@param pet boolean?
---@return boolean result
function MateAuras.IsSpellKnown(spell, pet)
  if spell == 0 or spell >= 2^31 then return false end
  if (pet) then
...
---@param spell string|number
---@return boolean result
function MateAuras.IsSpellKnownIncludingPet(spell)
  ...
  return MateAuras.IsSpellKnown(spell, false) or MateAuras.IsSpellKnown(spell, true)
end

if MateAuras.IsMists() then
  function MateAuras.IsGlyphActive(glyphID)
...
function MateAuras.GetNumSetItemsEquipped(setID)
...
---@return number result
function MateAuras.GetEffectiveAttackPower()
  local base, pos, neg = UnitAttackPower("player")
  return base + pos + neg
end

--- @type fun(): number
function MateAuras.GetEffectiveSpellPower()
...
local function valuesForTalentFunction(trigger)
  return function()
    ...
    elseif MateAuras.IsWrathOrCata() then
...
Private.load_prototype = {
  ...
  args = {
    ...
    },
    {
      ...
      type = "tristate",
      init = MateAuras.IsRetail() and "arg" or nil,
      width = MateAuras.normalWidth,
      optional = true,
      enable = MateAuras.IsRetail(),
      hidden = not MateAuras.IsRetail(),
    ...
    },
    {
      ...
      type = "tristate",
      init = MateAuras.IsWrathClassic() and "arg" or nil,
      width = MateAuras.normalWidth,
      optional = true,
      enable = MateAuras.IsWrathClassic(),
      hidden = not MateAuras.IsWrathClassic(),
    ...
    },
    {
      ...
      type = "tristate",
      init = MateAuras.IsMistsOrRetail() and "arg" or nil,
      width = MateAuras.normalWidth,
      optional = true,
      enable = MateAuras.IsWrathOrMistsOrRetail(),
      hidden = not MateAuras.IsWrathOrMistsOrRetail(),
      events = {"PET_BATTLE_OPENING_START", "PET_BATTLE_CLOSE"}
    },
    {
      name = "vehicle",
      display = (MateAuras.IsClassicEra()) and L["On Taxi"] or L["In Vehicle"],
      type = "tristate",
      init = "arg",
      width = MateAuras.normalWidth,
      optional = true,
      events = (MateAuras.IsClassicEra()) and {"UNIT_FLAGS"}
    ...
    },
    {
      ...
      type = "tristate",
      init = MateAuras.IsWrathOrCataOrMistsOrRetail() and "arg" or nil,
      width = MateAuras.normalWidth,
      optional = true,
      enable = MateAuras.IsWrathOrCataOrMistsOrRetail(),
      hidden = not MateAuras.IsWrathOrCataOrMistsOrRetail(),
    ...
    },
    {
      ...
      display = L["Hardcore"],
      type = "tristate",
      init = MateAuras.IsClassicEra() and "arg" or nil,
      width = MateAuras.normalWidth,
      enable = MateAuras.IsClassicEra(),
      hidden = not MateAuras.IsClassicEra(),
    },
    {
      name = "engraving",
    ...
    },
    {
      ...
      type = "multiselect",
      values = "class_types",
      init = not MateAuras.IsRetail() and "arg" or nil,
      enable = not MateAuras.IsRetail(),
      hidden = MateAuras.IsRetail(),
    },
    {
      name = "class_and_spec",
      ...
      init = MateAuras.IsCataOrMistsOrRetail() and "arg" or nil,
      enable = MateAuras.IsCataOrMistsOrRetail(),
      hidden = not MateAuras.IsCataOrMistsOrRetail(),
    ...
    },
    {
      ...
      test = MateAuras.IsRetail() and "MateAuras.CheckTalentId(%d) == (%d == 4)" or "MateAuras.CheckTalentByIndex(%d, %d)",
      enableTest = function(trigger, talent, arg)
        if MateAuras.IsRetail() then
          ...
          return MateAuras.CheckTalentByIndex(talent, arg) ~= nil
        end
      end,
      multiConvertKey = MateAuras.IsRetail() and function(trigger, key)
        local specId = Private.checkForSingleLoadCondition(trigger, "class_and_spec")
        if specId then
          local talentData = Private.GetTalentData(specId)
      ...
      events = (MateAuras.IsClassicOrWrathOrCataOrMists() and {"CHARACTER_POINTS_CHANGED", "PLAYER_TALENT_UPDATE", "ACTIVE_TALENT_GROUP_CHANGED"})
        or (MateAuras.IsRetail() and {"WA_TALENT_UPDATE"}),
      inverse = function(load)
        -- Check for multi select!
        return MateAuras.IsClassicEra() and (load.talent_extraOption == 2 or load.talent_extraOption == 3)
      end,
      extraOption = MateAuras.IsClassicEra() and {
        display = "",
        values = function()
          return Private.talent_extra_option_types
        end
      },
      control = MateAuras.IsWrathOrCataOrMistsOrRetail() and "MateAurasMiniTalent" or nil,
      multiNoSingle = MateAuras.IsWrathOrCataOrMistsOrRetail(), -- no single mode
      multiTristate = MateAuras.IsWrathOrCataOrMistsOrRetail(), -- values can be true/false/nil
      multiAll = MateAuras.IsWrathOrCataOrMistsOrRetail(), -- require all tests
      orConjunctionGroup = MateAuras.IsWrathOrCataOrMistsOrRetail() and "talent",
      multiUseControlWhenFalse = MateAuras.IsWrathOrCataOrMistsOrRetail(),
      enable = function(trigger)
        return MateAuras.IsClassicEra()
            or (MateAuras.IsWrathOrCataOrMists() and Private.checkForSingleLoadCondition(trigger, "class") ~= nil)
            or (MateAuras.IsRetail() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil)
      end,
      hidden = function(trigger)
        return not (
            MateAuras.IsClassicEra()
            or (MateAuras.IsWrathOrCataOrMists() and Private.checkForSingleLoadCondition(trigger, "class") ~= nil)
            or (MateAuras.IsRetail() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil))
      end,
    },
    {
      name = "talent2",
      display = MateAuras.IsWrathOrCataOrMistsOrRetail() and L["Or Talent"] or L["And Talent"],
      type = "multiselect",
      values = valuesForTalentFunction,
      test = MateAuras.IsRetail() and "MateAuras.CheckTalentId(%d) == (%d == 4)" or "MateAuras.CheckTalentByIndex(%d, %d)",
      enableTest = function(trigger, talent, arg)
      ...
      events = (MateAuras.IsClassicEra() and {"CHARACTER_POINTS_CHANGED"})
        or (MateAuras.IsWrathOrCataOrMists() and {"CHARACTER_POINTS_CHANGED", "PLAYER_TALENT_UPDATE"})
        or (MateAuras.IsRetail() and {"WA_TALENT_UPDATE"}),
      inverse = function(load)
        return MateAuras.IsClassicEra() and (load.talent2_extraOption == 2 or load.talent2_extraOption == 3)
      end,
      extraOption = MateAuras.IsClassicEra() and {
        display = "",
        values = function()
    ...
    },
    {
      ...
      events = (MateAuras.IsClassicEra() and {"CHARACTER_POINTS_CHANGED"})
        ...
        or (MateAuras.IsRetail() and {"WA_TALENT_UPDATE"}),
      inverse = function(load)
        return MateAuras.IsClassicEra() and (load.talent3_extraOption == 2 or load.talent3_extraOption == 3)
      end,
      extraOption = MateAuras.IsClassicEra() and {
        display = "",
        values = function()
    ...
    },
    {
      ...
      test = "MateAuras.CheckTalentId(%d) == (%d == 4)",
      enableTest = function(trigger, talent, arg)
        local specId = Private.checkForSingleLoadCondition(trigger, "class_and_spec")
        if specId then
      ...
      enable = function(trigger)
        return MateAuras.IsTWW() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil
      end,
      hidden = function(trigger)
        return not (MateAuras.IsTWW() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil)
      end,
    },
    {
      name = "pvptalent",
      ...
      test = "MateAuras.CheckPvpTalentBySpellId(%d)",
      enable = MateAuras.IsRetail(),
      hidden = not MateAuras.IsRetail(),
    ...
    },
    {
      ...
      test = "MateAuras.IsGlyphActive(%s)",
      events = {"GLYPH_ADDED", "GLYPH_REMOVED", "GLYPH_UPDATED", "USE_GLYPH"},
      enable = MateAuras.IsMists(),
      hidden = not MateAuras.IsMists(),
    },
    {
      name = "spellknown",
      display = L["Spell Known"],
      type = "spell",
      test = "MateAuras.IsSpellKnownForLoad(%s, %s)",
      events = MateAuras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED", "UNIT_PET", "PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED", "UNIT_PET"},
      showExactOption = true
    },
    {
      name = "not_spellknown",
      display = MateAuras.newFeatureString .. L["|cFFFF0000Not|r Spell Known"],
      type = "spell",
      test = "not MateAuras.IsSpellKnownForLoad(%s, %s)",
      events = MateAuras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED", "UNIT_PET", "PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED", "UNIT_PET"},
      showExactOption = true
    },
    {
    ...
    {
      name = "spec_position",
      display = MateAuras.newFeatureString .. L["Spec Position"],
      type = "multiselect",
    ...
    },
    {
      ...
      init = MateAuras.IsClassicOrWrathOrCataOrMists() and "arg" or nil,
      enable = MateAuras.IsClassicOrWrathOrCataOrMists(),
      hidden = MateAuras.IsRetail(),
    ...
    {
      name = "group_leader",
      display = MateAuras.newFeatureString .. L["Group Leader/Assist"],
      type = "multiselect",
      init = "arg",
    ...
    },
    {
      ...
      test = "MateAuras.CheckNumericIds(%q, encounterid)",
      events = {"ENCOUNTER_START", "ENCOUNTER_END"},
      optional = true,
    },
    {
    ...
    },
    {
      ...
      init = not MateAuras.IsClassicEra() and "arg" or nil,
      enable = not MateAuras.IsClassicEra(),
      hidden = MateAuras.IsClassicEra(),
    ...
    },
    {
      ...
      display = MateAuras.newFeatureString .. L["|cFFFF0000Not|r Item Equipped"],
      type = "item",
      multiEntry = {
        operator = "or"
      },
    ...
    },
    {
      ...
      type = "string",
      test = "MateAuras.CheckForItemBonusId(%q)",
      events = { "UNIT_INVENTORY_CHANGED", "PLAYER_EQUIPMENT_CHANGED"},
      desc = L["Supports multiple entries, separated by commas"]
    },
    {
      name = "not_item_bonusid_equipped",
      display =  MateAuras.newFeatureString .. L["|cFFFF0000Not|r Item Bonus Id Equipped"],
      type = "string",
      test = "not MateAuras.CheckForItemBonusId(%q)",
      events = { "UNIT_INVENTORY_CHANGED", "PLAYER_EQUIPMENT_CHANGED"},
      desc = L["Supports multiple entries, separated by commas"]
    }
  }
  ...
  elseif triggerUnit == "pet" then
    ...
    if Private.multiUnitUnits[triggerUnit] then
      ...
      for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
        isPet = MateAuras.UnitIsPet(unit)
  ...
  elseif triggerUnit == "pet" then
    ...
    if Private.multiUnitUnits[triggerUnit] then
      ...
      for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
        ...
        if (includePets ~= nil and isPet) or (includePets ~= "PetsOnly" and not isPet) then
          if unitisunit then
            MateAuras.WatchUnitChange(unitisunit)
          end
          MateAuras.WatchUnitChange(unit)
        end
      end
    else
      if unitisunit then
      ...
      MateAuras.WatchUnitChange(triggerUnit)
    end
  end
end
...
local function AddUnitSpecChangeInternalEvents(triggerUnit, t)
  if Private.multiUnitUnits[triggerUnit] then
    for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
      local isPet = MateAuras.UnitIsPet(unit)
      if (not isPet) then
...
local function AddUnitRoleChangeInternalEvents(triggerUnit, t)
  ...
  if Private.multiUnitUnits[triggerUnit] then
    for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
      if not MateAuras.UnitIsPet(unit) then
        tinsert(t, "UNIT_ROLE_CHANGED_" .. string.lower(unit))
      end
    end
  else
    if not MateAuras.UnitIsPet(triggerUnit) then
...
Private.event_prototypes = {
  ["Unit Characteristics"] = {
    ...
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local smart = %s
        local extraUnit = %q;
        local name, realm = MateAuras.UnitNameWithRealm(unit)
      ]=];
    ...
    args = {
      ...
      },
      {
        ...
        type = "multiselect",
        init = "MateAuras.SpecForUnit(unit)",
        values = "spec_types_all",
        store = true,
        conditionType = "select",
        enable = function(trigger)
          return MateAuras.IsCataOrMistsOrRetail() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end,
      ...
      },
      {
        ...
        sorted = true,
        conditionType = "select",
        enable = MateAuras.IsMistsOrRetail(),
        hidden = not MateAuras.IsMistsOrRetail(),
      },
      {
        name = "creatureFamily",
      ...
      },
      {
        ...
        display = L["Raid Role"],
        type = "select",
        init = "MateAuras.UnitRaidRole(unit)",
        values = "raid_role_types",
        store = true,
        conditionType = "select",
        enable = function(trigger)
          return MateAuras.IsClassicOrWrathOrCataOrMists() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end
      },
      -- {
      --   name = "raidMarkIndex",
      ...
      --   width = MateAuras.doubleWidth,
      --   enable = function(trigger)
      --     return MateAuras.IsRetail() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
      --   end,
      ...
      },
      {
        ...
        display = L["Hostility"],
        type = "select",
        init = "MateAuras.GetPlayerReaction(unit)",
      ...
      {
        hidden = true,
        test = "MateAuras.UnitExistsFixed(unit, smart) and specificUnitCheck"
      }
    },
  ...
  ["Faction Reputation"] = {
    ...
    args = {
      ...
      },
      {
        ...
        enable = MateAuras.IsRetail() and function(trigger)
  ...
  ["Power"] = {
    ...
    internal_events = function(trigger)
      ...
      if MateAuras.IsClassicOrWrath() and trigger.use_showCost and trigger.unit == "player" then
        tinsert(result, "WA_UNIT_QUEUED_SPELL_CHANGED");
      end
      if MateAuras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 26 then
        tinsert(result, "ECLIPSE_DIRECTION_CHANGE");
      end
      if MateAuras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 4 then
    ...
    init = function(trigger)
      ...
      table.insert(ret, ([=[
        ...
        if not MateAuras.IsRetail() and powerType == 99 then powerType = 1 end
      ...
      elseif MateAuras.IsMists() and powerType == 99 then
        table.insert(ret, ([[
          local power = UnitStagger(unit) or 0
          local scaleStagger = %s
          local total = math.max(1, UnitHealthMax(unit) * scaleStagger)
        ]]):format(trigger.use_scaleStagger and trigger.scaleStagger or 1))
      elseif MateAuras.IsMists() and (powerType == 14 or powerType == 7) then
        table.insert(ret, [[
      ...
      if (canEnableShowCost and trigger.use_showCost) then
        table.insert(ret, [[
          if (event == "UNIT_DISPLAYPOWER") then
            local cost = MateAuras.GetSpellCost(powerTypeToCheck)
    ...
    args = {
      ...
      },
      {
        ...
        reloadOptions = true,
        enable = function(trigger)
          return MateAuras.IsRetail() and trigger.unit == 'player' and trigger.use_powertype and trigger.powertype == 4
        end,
        hidden = not MateAuras.IsRetail()
      },
      {
      ...
      },
      {
        ...
        type = "string",
        validate = MateAuras.ValidateNumeric,
        enable = function(trigger)
      ...
      },
      {
        ...
        store = true,
        enable = function(trigger)
          return MateAuras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 26
        end,
      },
      {
        name = "power",
      ...
      {
        name = "maxpower",
        display = MateAuras.newFeatureString .. L["Max Power"],
        type = "number",
        init = "total",
        store = true,
        conditionType = "number",
      ...
      },
      {
        ...
        conditionType = "select",
        enable = function(trigger)
          return MateAuras.IsClassicOrCataOrMists() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end
      },
      -- {
      --   name = "raidMarkIndex",
    ...
    overlayFuncs = {
      ...
      },
      {
        ...
        enable = function(trigger)
          return MateAuras.IsRetail() and trigger.unit == 'player' and trigger.use_powertype and trigger.powertype == 4 and trigger.use_showChargedComboPoints
        end,
      },
    },
    automaticrequired = true
  ...
  ["Alternate Power"] = {
    ...
    init = function(trigger)
      ...
      local ret = [=[
        unit = string.lower(unit)
        local unitname, realm = MateAuras.UnitNameWithRealm(unit)
        local smart = %s
      ]=]
    ...
    args = {
      ...
      {
        hidden = true,
        test = "name and MateAuras.UnitExistsFixed(unit, smart) and specificUnitCheck"
      }
    },
    automaticrequired = true
  },
  ["Spell Activation Overlay"] = {
    ...
    loadFunc = function(trigger)
      if (trigger.use_exact_spellName) then
        MateAuras.WatchSpellActivation(tonumber(trigger.spellName));
      else
        MateAuras.WatchSpellActivation(type(trigger.spellName) == "number" and Private.ExecEnv.GetSpellName(trigger.spellName) or trigger.spellName);
      end
    end,
    init = function(trigger)
      local spellName
    ...
    args = {
      ...
      {
        hidden = true,
        test = "MateAuras.SpellActivationActive(spellName)";
      }
    },
    automaticrequired = true,
    progressType = "static"
  ...
  ["Cooldown Progress (Spell)"] = {
    ...
    loadFunc = function(trigger)
      ...
      MateAuras.WatchSpellCooldown(spellName, trigger.use_matchedRune, exactMatch, followoverride)
      if (trigger.use_showgcd) then
        MateAuras.WatchGCD();
      end
    end,
    ...
    args = {
      ...
      },
      {
        ...
        display = MateAuras.newFeatureString .. L["Show Loss of Control"],
        type = "toggle",
        test = "true",
        collapse = "extra Cooldown Progress (Spell)",
      },
      ...
      },
      {
        ...
        conditionType = "bool",
        conditionTest = function(state, needle)
          return state and state.show and (UnitExists('target') and state.spellname and MateAuras.IsSpellInRange(state.spellname, 'target') == needle)
        end,
        conditionEvents = AddTargetConditionEvents({
          "WA_SPELL_RANGECHECK",
        }),
  ...
  ["Cooldown Ready (Spell)"] = {
    ...
    loadFunc = function(trigger)
      ...
      local followoverride = not trigger.use_ignoreoverride
      MateAuras.WatchSpellCooldown(spellName, false, useExact, followoverride)
    end,
    init = function(trigger)
      local spellName = type(trigger.spellName) ~= "table" and trigger.spellName or 0
      if type(trigger.spellName) == "string" then
  ...
  ["Cooldown Progress (Item)"] = {
    ...
    loadFunc = function(trigger)
      MateAuras.WatchItemCooldown(trigger.itemName or 0)
      if (trigger.use_showgcd) then
        MateAuras.WatchGCD();
      end
    end,
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local showgcd = %s
        local startTime, duration, enabled, gcdCooldown = MateAuras.GetItemCooldown(itemname, showgcd);
    ...
    args = {
      ...
      {
        name = "itemId",
        display = MateAuras.newFeatureString .. L["ItemId"],
        hidden = true,
        init = "itemId",
        test = "true",
        store = true,
  ...
  ["Cooldown Progress (Equipment Slot)"] = {
    ...
    loadFunc = function(trigger)
      MateAuras.WatchItemSlotCooldown(trigger.itemSlot);
      if (trigger.use_showgcd) then
        MateAuras.WatchGCD();
      end
    end,
    init = function(trigger)
      local ret = [[
        local showgcd = %s
        local itemSlot = %s
        local startTime, duration, enable, gcdCooldown = MateAuras.GetItemSlotCooldown(itemSlot, showgcd)
        local expirationTime = startTime + duration
        local genericShowOn = %s
  ...
  ["Global Cooldown"] = {
    ...
    init = function(trigger)
      local ret = [[
        local inverse = %s;
        local duration, expirationTime, name, icon = MateAuras.GetGCDInfo()
        local hasSpellName = MateAuras.GcdSpellName();
      ]];
  ...
  ["Action Usable"] = {
    ...
    init = function(trigger)
      ...
      if trigger.use_ignoreSpellCooldown then
        ...
        ret = ret .. [=[
        local startTime, duration, gcdCooldown, readyTime, paused = MateAuras.GetSpellCooldown(effectiveSpellId, nil, nil, nil, nil)
        local charges, maxCharges, spellCount, chargeGainTime, chargeLostTime = MateAuras.GetSpellCharges(effectiveSpellId, nil)
      ...
      if(trigger.use_targetRequired) then
        ret = ret.."active = active and MateAuras.IsSpellInRange(spellName or '', 'target')\n";
      end
  ...
  ["Talent Known"] = {
    ...
    events = function()
      ...
      elseif MateAuras.IsWrathOrCataOrMists() then
        events = {
          "CHARACTER_POINTS_CHANGED",
          "SPELLS_CHANGED",
          "PLAYER_TALENT_UPDATE"
        }
      elseif MateAuras.IsRetail() then
        -- nothing
      end
      return {
        ["events"] = events
    ...
    internal_events = MateAuras.IsRetail() and  {"WA_TALENT_UPDATE"} or nil,
    force_events = (MateAuras.IsRetail() and "TRAIT_CONFIG_UPDATED") or "CHARACTER_POINTS_CHANGED",
    name = L["Talent Known"],
    init = function(trigger)
      local inverse = trigger.use_inverse and not MateAuras.IsMistsOrRetail()
      local ret = {}
          ...
          for index, value in pairs(trigger.talent.multi) do
            ...
            elseif MateAuras.IsRetail() then
              table.insert(ret, ([[
                ...
                local shouldBeActive = %s
                if talentId then
                  activeName, activeIcon, _, rank = MateAuras.GetTalentById(talentId)
      ...
      if (MateAuras.IsTWW() and trigger.use_herotalent == false) then
    ...
    args = {
      ...
      },
      {
        ...
        type = "select",
        init = "MateAuras.IsRetail() and Private.ExecEnv.GetSpecialization()",
        required = true,
        values = function(trigger)
          return MateAuras.spec_types_specific[trigger.class]
        end,
        enable = function(trigger)
          if MateAuras.IsRetail() and trigger.use_class and trigger.class then
            return true
          else
            return false
          end
      ...
      },
      {
        ...
        multiUseControlWhenFalse = MateAuras.IsMistsOrRetail(),
        multiAll = MateAuras.IsMistsOrRetail(),
        multiNoSingle = MateAuras.IsMistsOrRetail(),
        multiTristate = MateAuras.IsMistsOrRetail(), -- values can be true/false/nil
        control = MateAuras.IsMistsOrRetail() and "MateAurasMiniTalent" or nil,
        multiConvertKey = MateAuras.IsRetail() and function(trigger, key)
          local classId
          for i = 1, GetNumClasses() do
            if select(2, GetClassInfo(i)) == trigger.class then
      ...
      },
      {
        ...
        type = "multiselect",
        values = function(trigger)
          if not MateAuras.IsTWW() then return {} end
          local classId
          for i = 1, GetNumClasses() do
            if select(2, GetClassInfo(i)) == trigger.class then
              classId = i
        ...
        enable = function(trigger)
          return MateAuras.IsTWW() and trigger.use_class and trigger.class and trigger.use_spec and trigger.spec and true or false
        end,
        test = "heroActive",
        reloadOptions = true,
      },
      {
        ...
        type = "toggle",
        test = "true",
        enable = not MateAuras.IsMistsOrRetail(),
        hidden = MateAuras.IsMistsOrRetail(),
      },
      {
        hidden = true,
        name = "icon",
    ...
    init = function(trigger)
      ...
      if (trigger.use_talent) then
        ...
        table.insert(ret, ([[
          ...
          local active, spellId = MateAuras.CheckPvpTalentBySpellId(index)
          if spellId then
        ...
        if (trigger.talent.multi) then
          ...
          for index in pairs(trigger.talent.multi) do
            table.insert(ret, ([[
              if (not active) then
                local index = %s
                active, spellId = MateAuras.CheckPvpTalentBySpellId(index)
  ...
  ["Totem"] = {
    ...
    triggerFunction = function(trigger)
      ...
      ret = ret:format(trigger.use_totemType and tonumber(trigger.totemType) or "nil",
        ...
        MateAuras.IsRetail() and trigger.use_totemSpellId and trigger.totemSpellId or "nil",
        MateAuras.IsRetail() and not trigger.use_ignoreoverride and "true" or "false",
    ...
    args = {
      ...
      },
      {
        ...
        test = "true",
        enable = function(trigger)
          return trigger.use_totemSpellId and MateAuras.IsRetail()
        end,
      },
      {
        name = "icon",
  ...
  ["Item Count"] = {
    ...
    loadFunc = function(trigger)
      if(trigger.use_includeCharges) then
        MateAuras.RegisterItemCountWatch();
      end
    end,
    init = function(trigger)
      local itemName = type(trigger.itemName) == "number" and trigger.itemName or string.format("%q", trigger.itemName or "0")
      local ret = [[
        ...
        if MateAuras.IsRetail() and itemName then
      ...
      return ret:format(
        ...
        MateAuras.IsRetail() and trigger.use_includeWarBandBank and "true" or "nil"
      )
    end,
    args = {
      {
  ...
  ["Stance/Form/Aura"] = {
    ...
    nameFunc = function(trigger)
      ...
      if(class == trigger.class) then
        ...
        local types = MateAuras[class:lower().."_form_types"];
        if(types) then
  ...
  ["Weapon Enchant"] = {
    ...
    name = MateAuras.IsRetail() and L["Weapon Enchant / Fishing Lure"] or L["Weapon Enchant"],
    init = function(trigger)
      MateAuras.TenchInit();
      ...
      local ret = [[
        ...
        if triggerWeaponType == "main" then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = MateAuras.GetMHTenchInfo()
        elseif triggerWeaponType == "off" then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = MateAuras.GetOHTenchInfo()
        elseif triggerWeaponType == "ranged" and MateAuras.IsCataClassic() then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = MateAuras.GetRangeTenchInfo()
        end
    ...
    args = {
      ...
      },
      {
        ...
        test = "true",
        enable = function(trigger)
          return not MateAuras.IsRetail() and (not trigger.showOn or trigger.showOn == "showOnActive")
        end,
        hidden = MateAuras.IsRetail(),
        store = true
      },
  ...
  ["Chat Message"] = {
    ...
    args = {
      ...
      },
      {
        ...
        type = "toggle",
        test = "true",
        init = "use_cloneId and MateAuras.GetUniqueCloneId() or ''",
        reloadOptions = true
      },
    },
    countEvents = true,
  ...
  ["Encounter Events"] = {
    ...
    args = {
      ...
      },
      {
        ...
        type = "select",
        values = "difficulty_types",
        test = "%q == MateAuras.InstanceDifficulty()",
        conditionType = "select",
        conditionTest = function(state, needle)
          return MateAuras.InstanceDifficulty() == needle
        end,
        store = true,
        init = "arg"
      },
  ...
  ["Evoker Essence"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        local genericShowOn = %s
        local duration, expirationTime, remaining, paused, essence, total = MateAuras.GetEssenceCooldown(triggerEssence)
        local ready = paused
  ...
  ["Death Knight Rune"] = {
    ...
    loadFunc = function(trigger)
      ...
      if (trigger.use_rune) then
        MateAuras.WatchRuneCooldown(trigger.rune);
      else
        for i = 1, 6 do
          MateAuras.WatchRuneCooldown(i);
        end
      end
    end,
    init = function(trigger)
      ...
      if MateAuras.IsWrathOrCataOrMists() then
        ret = [[
          ...
          local startTime, duration = MateAuras.GetRuneCooldown(rune);
        ...
        ret = [[
          ...
          local numRunes = 0;
          for index = 1, 6 do
            local startTime = MateAuras.GetRuneCooldown(index);
            if startTime == 0 then
              numRunes = numRunes + 1;
            end
          end
    ...
    args = {
      ...
      },
      {
        ...
        store = true,
        conditionType = "bool",
        enable = function(trigger) return MateAuras.IsWrathOrCataOrMists() and trigger.use_rune end,
        hidden = not MateAuras.IsWrathOrCataOrMists()
      },
      {
        name = "remaining",
        display = L["Remaining Time"],
      ...
      },
      {
        ...
        store = true,
        conditionType = "number",
        enable = function(trigger) return MateAuras.IsWrathOrCataOrMists() and not trigger.use_rune end,
        hidden = not MateAuras.IsWrathOrCataOrMists()
      },
      {
        name = "frostRunes",
      ...
      },
      {
        ...
        type = "toggle",
        test = "true",
        enable = function(trigger) return MateAuras.IsWrathOrCataOrMists() and trigger.use_bloodRunes or trigger.use_unholyRunes or trigger.use_frostRunes end,
        hidden = not MateAuras.IsWrathOrCataOrMists()
      },
    },
    durationFunc = function(trigger)
      if trigger.use_rune then
        local startTime, duration = MateAuras.GetRuneCooldown(trigger.rune)
  ...
  ["Item Equipped"] = {
    ...
    init = function(trigger)
      ...
      ret = ret ..[[
        ...
        local equipped = MateAuras.CheckForItemEquipped(triggerItemName, itemSlot)
      ]]
    ...
    args = {
      ...
      {
        name = "itemSlot",
        display = MateAuras.newFeatureString .. L["Item Slot"],
        type = "select",
        values = "item_slot_types",
        test = "true",
      },
  ...
  ["Item Bonus Id Equipped"] = {
    ...
    init = function(trigger)
      local ret = [=[
        ...
        local itemBonusId, itemId, itemName, icon, itemSlot, itemSlotString = MateAuras.GetBonusIdInfo(item, useItemSlot and slotSelected)
        local itemBonusId = tonumber(itemBonusId)
        if fetchLegendaryPower then
          itemName, icon = MateAuras.GetLegendaryData(itemBonusId or item)
        end
  ...
  ["Item Set"] = {
    ...
    init = function(trigger)
      local ret = [[
        local setid = %s
        local value, total, name = MateAuras.GetNumSetItemsEquipped(setid)
      ]]
      return string.format(ret, trigger.itemSetId and tonumber(trigger.itemSetId) or "0");
    end,
    GetNameAndIcon = function(trigger)
      local name = select(3, MateAuras.GetNumSetItemsEquipped(trigger.itemSetId and tonumber(trigger.itemSetId) or 0))
      return name, nil
    end,
    statesParameter = "one",
    args = {
      {
        ...
        desc = function()
          ...
          elseif MateAuras.IsClassicEra() then
  ...
  ["Equipment Set"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        local itemSetName, icon, numEquipped, numItems = MateAuras.GetEquipmentSetInfo(useItemSetName and triggerItemSetName or nil, partial);
      ]];
    ...
    GetNameAndIcon = function(trigger)
      local name, icon = MateAuras.GetEquipmentSetInfo(trigger.use_itemSetName and trigger.itemSetName or nil, true)
      return name, icon
    end,
    statesParameter = "one",
    args = {
  ...
  ["Threat Situation"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        if unit and unit ~= "none" then
          aggro, status, threatpct, rawthreatpct, threatvalue = MateAuras.UnitDetailedThreatSituation('player', unit)
    ...
    args = {
      ...
      {
        hidden = true,
        test = "MateAuras.UnitExistsFixed(unit, false) and specificUnitCheck"
      }
    },
    automaticrequired = true
  },
  ...
  ["Cast"] = {
    ...
    loadFunc = function(trigger)
      if trigger.use_showLatency and trigger.unit == "player" then
        MateAuras.WatchForCastLatency()
      end
      if trigger.unit == "nameplate" and trigger.use_onUpdateUnitTarget then
        MateAuras.WatchForNameplateTargetChange()
      end
      local includePets = trigger.use_includePets == true and trigger.includePets or nil
      AddWatchedUnits(trigger.unit, includePets)
    end,
    ...
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local sourceName, sourceRealm = MateAuras.UnitNameWithRealm(unit)
        local destName, destRealm = MateAuras.UnitNameWithRealm(destUnit)
        ...
        spell, _, icon, startTime, endTime, _, _, interruptible, spellId = MateAuras.UnitCastingInfo(unit)
        ...
        if spell then
          ...
          spell, _, icon, startTime, endTime, _, interruptible, spellId, _, stageTotal = MateAuras.UnitChannelInfo(unit)
           if issecretvalue(spell) then
            return false
          end
          if spell then
    ...
    args = {
      ...
      },
      {
        ...
        enable = MateAuras.IsRetail() and function(trigger) return not trigger.use_inverse end or false,
        hidden = not MateAuras.IsRetail()
      },
      {
        name = "empowered",
      ...
      },
      {
        ...
        display = MateAuras.newFeatureString .. L["Advanced Caster's Target Check"],
      ...
      {
        hidden = true,
        test = "MateAuras.UnitExistsFixed(unit, smart) and ((not inverseTrigger and spell) or (inverseTrigger and not spell)) and specificUnitCheck"
      },
      {
        name = "stagesData",
        hidden = true,
    ...
    overlayFuncs = {
      {
        name = L["Latency"],
        func = function(trigger, state)
          local latency = MateAuras.GetCastLatency()
      ...
      },
      {
        ...
        enable = function(trigger)
          return MateAuras.IsRetail() and trigger.use_showChargedDuration
        end
      }
    },
    GetNameAndIcon = function(trigger)
      ...
      if trigger.use_spellNames and type(trigger.spellNames) == "table" then
        for _, spellName in ipairs(trigger.spellNames) do
          spellId = MateAuras.SafeToNumber(spellName)
          if spellId then
      ...
      if trigger.use_spellIds and type(trigger.spellIds) == "table" then
        for _, spellIdString in ipairs(trigger.spellIds) do
          spellId = MateAuras.SafeToNumber(spellIdString)
          if spellId then
  ...
  ["Character Stats"] = {
    ...
    loadFunc = function(trigger)
      if trigger.use_moveSpeed then
        MateAuras.WatchForPlayerMoving()
      end
    end,
    init = function()
      local ret = [[
    ...
    args = {
      ...
      },
      {
        ...
        store = true,
        enable = MateAuras.IsWrathOrCataOrMists(),
        conditionType = "number",
        hidden = not MateAuras.IsWrathOrCataOrMists(),
        multiEntry = {
          operator = "and",
      ...
      },
      {
        ...
        display = L["Mastery (%)"],
        type = "number",
        init = "MateAuras.IsCataOrMists() and GetMastery() or GetMasteryEffect()",
        store = true,
        enable = MateAuras.IsCataOrMistsOrRetail(),
        conditionType = "number",
        hidden = not MateAuras.IsCataOrMistsOrRetail(),
      ...
      },
      {
        ...
        type = "number",
        init = "MateAuras.GetEffectiveAttackPower()",
        store = true,
        conditionType = "number",
        multiEntry = {
          operator = "and",
      ...
      },
      {
        ...
        type = "number",
        init = "MateAuras.GetEffectiveSpellPower()",
        store = true,
        conditionType = "number",
        multiEntry = {
          operator = "and",
        ...
        enable = MateAuras.IsClassicOrCataOrMists(),
        hidden = not MateAuras.IsClassicOrCataOrMists(),
      },
      {
        type = "header",
      ...
      },
      {
        ...
        type = "number",
        init = MateAuras.IsWrathClassic() and "GetCombatRating(CR_RESILIENCE_PLAYER_DAMAGE_TAKEN)"
      ...
      },
      {
        ...
        type = "number",
        init = MateAuras.IsWrathClassic() and "GetCombatRatingBonus(CR_RESILIENCE_PLAYER_DAMAGE_TAKEN)"
  ...
  ["Conditions"] = {
    ...
    args = {
      ...
      },
      {
        ...
        enable = MateAuras.IsWrathOrRetail(),
        hidden = not MateAuras.IsWrathOrRetail()
      },
      {
        name = "alive",
        display = L["Alive"],
      ...
      {
        name = "vehicle",
        display = not MateAuras.IsWrathOrCataOrMistsOrRetail() and L["On Taxi"] or L["In Vehicle"],
        type = "tristate",
        init = not MateAuras.IsWrathOrCataOrMistsOrRetail() and "UnitOnTaxi('player')" or "UnitInVehicle('player')",
      },
      {
        name = "resting",
        display = L["Resting"],
      ...
      },
      {
        ...
        type = "multiselect",
        values = "difficulty_types",
        init = "MateAuras.InstanceDifficulty()",
        enable = MateAuras.IsRetail(),
        hidden = not MateAuras.IsRetail(),
      },
      {
        ...
        type = "multiselect",
        values = "instance_difficulty_types",
        init = "MateAuras.InstanceTypeRaw()",
        enable = MateAuras.IsRetail(),
        hidden = not MateAuras.IsRetail(),
      },
      {
        ...
        display = L["Secret Restrictions Active"],
        type = "tristate",
        init = "MateAuras.IsSecretStateActive()",
      },
    },
    automaticrequired = true,
    progressType = "none"
  ...
  ["Spell Known"] = {
    type = "spell",
    events = {
      ["events"] = MateAuras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED","PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED"},
      ["unit_events"] = {
        ["player"] = {"UNIT_PET"}
      }
    },
    ...
    init = function(trigger)
      ...
      if (trigger.use_inverse) then
        table.insert(ret, ([[
          local usePet = %s;
          local active = not spellName or not MateAuras.IsSpellKnown(spellName, usePet)
        ]]):format(trigger.use_petspell and "true" or "false"))
      else
        table.insert(ret, ([[
          local usePet = %s;
          local active = spellName and MateAuras.IsSpellKnown(spellName, usePet)
        ]]):format(trigger.use_petspell and "true" or "false"))
      end
      return table.concat(ret)
    end,
    ...
    args = {
      ...
      {
        name = "inverse",
        display = MateAuras.newFeatureString .. L["Inverse"],
        type = "toggle",
        test = "true",
      },
      {
  ...
  ["Money"] = {
    ...
    internal_events = {"WA_DELAYED_PLAYER_ENTERING_WORLD"},
    force_events = "WA_DELAYED_PLAYER_ENTERING_WORLD",
    name = MateAuras.newFeatureString..L["Player Money"],
    init = function()
      return [=[
  ...
  ["Currency"] = {
    ...
    internal_events = {"WA_DELAYED_PLAYER_ENTERING_WORLD"},
    force_events = "WA_DELAYED_PLAYER_ENTERING_WORLD",
    name = MateAuras.newFeatureString..L["Currency"],
    triggerFunction = function(trigger)
  ...
  ["Location"] = {
    ...
    name = MateAuras.newFeatureString..L["Location"],
    init = function(trigger)
      local ret = [=[
...
if MateAuras.IsCataOrMists() then
