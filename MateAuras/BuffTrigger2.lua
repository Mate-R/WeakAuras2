...
local newAPI = MateAuras.IsRetail()
...
local function UnitInSubgroupOrPlayer(unit, includePets)
  ...
  elseif includePets == "PlayersAndPets" then
    return UnitInSubgroup(MateAuras.petUnitToUnit[unit] or unit) or UnitIsUnit("player", unit) or UnitIsUnit("pet", unit)
  elseif includePets == "PetsOnly" then
    return UnitInSubgroup(MateAuras.petUnitToUnit[unit]) or UnitIsUnit("pet", unit)
  end
end
...
local function UpdateToolTipDataInMatchData(matchData, time)
  ...
  if matchData.unit and matchData.auraInstanceID then
    local dataInstanceID, tooltip, _, tooltip1, tooltip2, tooltip3, tooltip4 = MateAuras.GetAuraInstanceTooltipInfo(matchData.unit, matchData.auraInstanceID, matchData.filter)
  ...
  elseif matchData.unit and matchData.index and matchData.filter then
    local tooltip, _, tooltip1, tooltip2, tooltip3, tooltip4 = MateAuras.GetAuraTooltipInfo(matchData.unit, matchData.index, matchData.filter)
    changed = matchData.tooltip ~= tooltip or matchData.tooltip1 ~= tooltip1
      or matchData.tooltip2 ~= tooltip2 or matchData.tooltip3 ~= tooltip3 or matchData.tooltip4 ~= tooltip4
    matchData.tooltip, matchData.tooltip1, matchData.tooltip2, matchData.tooltip3, matchData.tooltip4 = tooltip, tooltip1, tooltip2, tooltip3, tooltip4
  end
...
local function UpdateMatchData(time, matchDataChanged, unit, index, auraInstanceID, filter, name, icon, stacks, debuffClass, duration, expirationTime, unitCaster, isStealable, isBossDebuff, isCastByPlayer, spellId, modRate, points)
  ...
  local debuffClassIcon = MateAuras.EJIcons[debuffClass]
...
local function UpdateStateWithMatch(time, bestMatch, triggerStates, cloneId, matchCount, unitCount, maxUnitCount, matchCountPerUnit, totalStacks, affected, affectedUnits, unaffected, unaffectedUnits, role, raidMark)
  local debuffClassIcon = MateAuras.EJIcons[bestMatch.debuffClass]
...
local function UpdateStateWithNoMatch(time, triggerStates, triggerInfo, cloneId, unit, matchCount, unitCount, maxUnitCount, matchCountPerUnit, totalStacks, affected, affectedUnits, unaffected, unaffectedUnits, role, raidMark)
  local fallbackName, fallbackIcon = BuffTrigger.GetNameAndIconSimple(MateAuras.GetData(triggerInfo.id), triggerInfo.triggernum)
  if not triggerStates[cloneId] then
    triggerStates[cloneId] = {
      show = true,
      changed = true,
    ...
    if allUnits then
      ...
      return function()
        if raid then
          if i <= 40 then
            ...
            if includePets == "PlayersAndPets" then
              ret = pets and MateAuras.raidpetUnits[i] or MateAuras.raidUnits[i]
              pets = not pets
              if pets then
                i = i + 1
              end
            elseif includePets == "PetsOnly" then
              ret = MateAuras.raidpetUnits[i]
              i = i + 1
            else -- raid
              ret = MateAuras.raidUnits[i]
              i = i + 1
            end
            return ret
          end
        ...
        if i <= 4 then
          local ret
          if includePets == "PlayersAndPets" then
            ret = pets and MateAuras.partypetUnits[i] or MateAuras.partyUnits[i]
            pets = not pets
            if pets then
              i = i + 1
            end
          elseif includePets == "PetsOnly" then
            ret = MateAuras.partypetUnits[i]
            i = i + 1
          else -- group
            ret = MateAuras.partyUnits[i]
            i = i + 1
          end
          return ret
        end
...
local function TriggerInfoApplies(triggerInfo, unit)
  ...
  if MateAuras.UnitIsPet(unit) then
    controllingUnit = MateAuras.petUnitToUnit[unit]
  end
  ...
  if triggerInfo.raidRole then
    local raidRole = MateAuras.UnitRaidRole(controllingUnit)
  ...
  if triggerInfo.hostility and MateAuras.GetPlayerReaction(unit) ~= triggerInfo.hostility then
    return false
  end
  ...
  if triggerInfo.nameChecker and not triggerInfo.nameChecker:Check(MateAuras.UnitNameWithRealm(unit)) then
    return false
  end
...
Private.frames["MateAuras Buff2 Frame"] = Buff2Frame
...
local function EventHandler(frame, event, arg1, arg2, ...)
  ...
  elseif event == "UNIT_PET" then
    local pet = MateAuras.unitToPetUnit[arg1]
    if pet then
...
if MateAuras.IsCataOrMistsOrRetail() then
  Private.LibSpecWrapper.Register(function(unit)
    ...
    local deactivatedTriggerInfos = {}
    RecheckActiveForUnitType("group", unit, deactivatedTriggerInfos)
    RecheckActiveForUnitType("group", MateAuras.unitToPetUnit[unit], deactivatedTriggerInfos)
    DeactivateScanFuncs(deactivatedTriggerInfos)
...
local function createScanFunc(trigger)
  ...
  if use_ignore_name then
    ...
    for index, spellName in ipairs(trigger.ignoreAuraNames) do
      local spellId = MateAuras.SafeToNumber(spellName)
  ...
  if use_ignore_spellId then
    ...
    for index, spellId in ipairs(trigger.ignoreAuraSpellids) do
      local spell = MateAuras.SafeToNumber(spellId)
      if spell then
  ...
  return {
    untrackableSoftTarget = {
      ...
      check = function(trigger)
        return MateAuras.IsUntrackableSoftTarget(trigger.unit)
      end
    }
  }
end
...
function BuffTrigger.Add(data)
  ...
  for triggernum, triggerData in ipairs(data.triggers) do
    ...
    if trigger.type == "aura2" then
      ...
      local effectiveGroupRole = MateAuras.IsWrathOrCataOrMistsOrRetail() and (groupTrigger and trigger.useGroupRole and trigger.group_role) or nil
      local effectiveRaidRole = MateAuras.IsClassicOrWrathOrCataOrMists() and (groupTrigger and trigger.useRaidRole and trigger.raid_role) or nil
      local effectiveClass = groupTrigger and trigger.useClass and trigger.class
      local effectiveSpecId = MateAuras.IsCataOrMistsOrRetail() and (groupTrigger and trigger.useActualSpec and trigger.actualSpec) or nil
      local effectiveArenaSpec = MateAuras.IsRetail() and (trigger.unit == "arena" and trigger.useArenaSpec and trigger.arena_spec) or nil
      ...
      -- local effectiveInRange = MateAuras.IsRetail() and groupTrigger and trigger.inRange
      ...
      local triggerInformation = {
        ...
        fetchRole = MateAuras.IsCataOrMistsOrRetail() and trigger.unit ~= "multi" and trigger.fetchRole,
...
function BuffTrigger.GetNameAndIcon(data, triggernum)
  ...
  if (not name or not icon and MateAuras.spellCache) then
    local trigger = data.triggers[triggernum].trigger
    if trigger.useName and trigger.auranames then
      for index, spellName in ipairs(trigger.auranames) do
        icon = MateAuras.spellCache.GetIcon(spellName)
        if icon then
...
function BuffTrigger.GetAdditionalProperties(data, triggernum)
  ...
  if MateAuras.IsCataOrMistsOrRetail() and trigger.unit ~= "multi" and trigger.fetchRole then
...
function BuffTrigger.GetTriggerConditions(data, triggernum)
  ...
  result["nameCaster"] = {
    ...
    test = function(state, needle, op, preamble)
      return state.unitCaster and preamble:Check(MateAuras.UnitNameWithRealm(state.unitCaster))
    end,
    operator_types = "none",
  }
...
function BuffTrigger.GetTriggerDescription(data, triggernum, namestable)
  ...
  if trigger.useName and trigger.auranames then
    for index, name in pairs(trigger.auranames) do
      ...
      local icon
      local spellId = MateAuras.SafeToNumber(name)
      if spellId then
        icon = Private.ExecEnv.GetSpellIcon(spellId)
      else
        icon = MateAuras.spellCache.GetIcon(name)
      end
...
MateAuras.RegisterTriggerSystem({"aura2"}, BuffTrigger)
