...
local ADDON_NAME = "MateAuras"
---@class MateAuras
local MateAuras = MateAuras
local L = MateAuras.L
local versionString = MateAuras.versionString
local prettyPrint = MateAuras.prettyPrint

MateAurasTimers = setmetatable({}, {__tostring=function() return "MateAuras" end})
LibStub("AceTimer-3.0"):Embed(MateAurasTimers)
...
function MateAurasTimers:ScheduleTimerFixed(func, delay, ...)
  if (delay < Private.maxTimerDuration) then
    if delay + GetTime() > maxUpTime then
      MateAuras.prettyPrint(MateAuras.L["Can't schedule timer with %i, due to a World of Warcraft bug with high computer uptime. (Uptime: %i). Please restart your computer."]:format(delay, GetTime()))
      return
    end
...
if CustomNames then
  MateAuras.GetName = CustomNames.Get
  MateAuras.UnitName = CustomNames.UnitName
  MateAuras.GetUnitName = CustomNames.GetUnitName
...
local timer = MateAurasTimers
MateAuras.timer = timer

local loginQueue = {}
local queueshowooc

function MateAuras.InternalVersion()
...
do
  ...
  local function waErrorHandler(errorMessage)
    ...
    if currentErrorHandlerId then
      data = MateAuras.GetData(currentErrorHandlerId)
    ...
    table.insert(juicedMessage, L["MateAuras Version: %s"]:format(MateAuras.versionString))
...
function Private.LoadOptions(msg)
  if not(C_AddOns.IsAddOnLoaded("MateAurasOptions")) then
    if not MateAuras.IsLoginFinished() then
      prettyPrint(Private.LoginMessage())
      loginQueue[#loginQueue + 1] = MateAuras.OpenOptions
    elseif InCombatLockdown() then
      ...
      local loaded, reason = C_AddOns.LoadAddOn("MateAurasOptions");
      if not(loaded) then
        reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
        MateAuras.prettyPrint(string.format(L["Options could not be loaded, the addon is %s"], reason));
        return false;
      end
    end
  end
...
function MateAuras.OpenOptions(msg)
  if Private.NeedToRepairDatabase() then
    StaticPopup_Show("MateAuras_CONFIRM_REPAIR", nil, nil, {reason = "downgrade"})
  elseif (MateAuras.IsLoginFinished() and Private.LoadOptions(msg)) then
    MateAuras.ToggleOptions(msg, Private);
  end
end
...
SLASH_MateAuras1, SLASH_MateAuras2 = "/wa", "/mtw";
function SlashCmdList.MateAuras(input)
  local args, msg = {}, nil
  ...
  if msg == "pstart" then
    MateAuras.StartProfile(args[1]);
  elseif msg == "pstop" then
    MateAuras.StopProfile();
  elseif msg == "pprint" then
    MateAuras.PrintProfile();
  elseif msg == "pcancel" then
    MateAuras.CancelScheduledProfile()
  ...
  elseif msg == "ff" or msg == "feat" or msg == "feature" then
    ...
    MateAuras.OpenOptions(msg);
  end
end

if not MateAuras.IsLibsOK() then return end

function MateAuras.ToggleMinimap()
  MateAurasSaved.minimap.hide = not MateAurasSaved.minimap.hide
  if MateAurasSaved.minimap.hide then
    LDBIcon:Hide("MateAuras");
    prettyPrint(L["Use /wa minimap to show the minimap icon again."])
  else
    LDBIcon:Show("MateAuras");
  end
end

BINDING_HEADER_MateAuras = ADDON_NAME
BINDING_NAME_MateAurasTOGGLE = L["Toggle Options Window"]
BINDING_NAME_MateAurasPROFILINGTOGGLE = L["Toggle Performance Profiling Window"]
BINDING_NAME_MateAurasPRINTPROFILING = L["Print Profiling Results"]

-- An alias for MateAurasSaved, the SavedVariables
-- Noteable properties:
--  debug: If set to true, WeakAura.debug() outputs messages to the chat frame
--  displays: All aura settings, keyed on their id

---@class MateAurasSaved
local db;

-- While true no events are handled. E.g. MateAuras is paused while the Options dialog is open
local paused = true;
...
MateAuras.raidUnits = {};
MateAuras.raidpetUnits = {};
MateAuras.partyUnits = {};
MateAuras.partypetUnits = {};
MateAuras.petUnitToUnit = {
  pet = "player"
}
MateAuras.unitToPetUnit = {
  player = "pet"
}
do
  for i=1,40 do
    MateAuras.raidUnits[i] = "raid"..i
    MateAuras.raidpetUnits[i] = "raidpet"..i
    MateAuras.petUnitToUnit["raidpet"..i] = "raid"..i
    MateAuras.unitToPetUnit["raid"..i] = "raidpet"..i
  end
  for i=1,4 do
    MateAuras.partyUnits[i] = "party"..i
    MateAuras.partypetUnits[i] = "partypet"..i
    MateAuras.petUnitToUnit["partypet"..i] = "party"..i
    MateAuras.unitToPetUnit["party"..i] = "partypet"..i
  end
end

---@param unit UnitToken
---@return boolean isPet
MateAuras.UnitIsPet = function(unit)
  return MateAuras.petUnitToUnit[unit] ~= nil
end
...
function MateAuras.RegisterSubRegionType(name, displayName, supportFunction, createFunction, modifyFunction, onAcquire, onRelease, default, addDefaultsForNewAura, properties, supportsAdd)
  if not(name) then
    error("Improper arguments to MateAuras.RegisterSubRegionType - name is not defined", 2);
  elseif(type(name) ~= "string") then
    error("Improper arguments to MateAuras.RegisterSubRegionType - name is not a string", 2);
  elseif not(displayName) then
    error("Improper arguments to MateAuras.RegisterSubRegionType - display name is not defined".." "..name, 2);
  elseif(type(displayName) ~= "string") then
    error("Improper arguments to MateAuras.RegisterSubRegionType - display name is not a string", 2);
  elseif not(supportFunction) then
    error("Improper arguments to MateAuras.RegisterSubRegionType - support function is not defined", 2);
  elseif(type(supportFunction) ~= "function") then
    error("Improper arguments to MateAuras.RegisterSubRegionType - support function is not a function", 2);
  elseif not(createFunction) then
    error("Improper arguments to MateAuras.RegisterSubRegionType - creation function is not defined", 2);
  elseif(type(createFunction) ~= "function") then
    error("Improper arguments to MateAuras.RegisterSubRegionType - creation function is not a function", 2);
  elseif not(modifyFunction) then
    error("Improper arguments to MateAuras.RegisterSubRegionType - modification function is not defined", 2);
  elseif(type(modifyFunction) ~= "function") then
    error("Improper arguments to MateAuras.RegisterSubRegionType - modification function is not a function", 2)
  elseif not(onAcquire) then
    error("Improper arguments to MateAuras.RegisterSubRegionType - onAcquire function is not defined", 2);
  elseif(type(onAcquire) ~= "function") then
    error("Improper arguments to MateAuras.RegisterSubRegionType - onAcquire function is not a function", 2)
  elseif not(onRelease) then
    error("Improper arguments to MateAuras.RegisterSubRegionType - onRelease function is not defined", 2);
  elseif(type(onRelease) ~= "function") then
    error("Improper arguments to MateAuras.RegisterSubRegionType - onRelease function is not a function", 2)
  elseif not(default) then
    error("Improper arguments to MateAuras.RegisterSubRegionType - default options are not defined", 2);
  elseif(type(default) ~= "table" and type(default) ~= "function") then
    error("Improper arguments to MateAuras.RegisterSubRegionType - default options are not a table or a function", 2);
  elseif(addDefaultsForNewAura and type(addDefaultsForNewAura) ~= "function") then
    error("Improper arguments to MateAuras.RegisterSubRegionType - addDefaultsForNewAura function is not nil or a function", 2)
  elseif(subRegionTypes[name]) then
    error("Improper arguments to MateAuras.RegisterSubRegionType - region type \""..name.."\" already defined", 2);
  else
...
function MateAuras.RegisterSubRegionOptions(name, createFunction, description, getAnchors)
  if not(name) then
    error("Improper arguments to MateAuras.RegisterSubRegionOptions - name is not defined", 2);
  elseif(type(name) ~= "string") then
    error("Improper arguments to MateAuras.RegisterSubRegionOptions - name is not a string", 2);
  elseif not(createFunction) then
    error("Improper arguments to MateAuras.RegisterSubRegionOptions - creation function is not defined", 2);
  elseif(type(createFunction) ~= "function") then
    error("Improper arguments to MateAuras.RegisterSubRegionOptions - creation function is not a function", 2);
  elseif(getAnchors and type(getAnchors) ~= "function") then
    error("Improper arguments to MateAuras.RegisterSubRegionOptions - getAnchors function is not a function", 2);
  elseif(subRegionOptions[name]) then
    error("Improper arguments to MateAuras.RegisterSubRegionOptions - region type \""..name.."\" already defined", 2);
  else
...
---@diagnostic disable-next-line: duplicate-set-field (it's replaced in MateAurasOptions.lua)
function MateAuras.IsOptionsOpen()
  return false;
end
...
function MateAuras.GetActiveConditions(id, cloneId)
  ...
  if (data.actions) then
    if data.actions.init then
      if data.actions.init.do_custom and data.actions.init.custom then
        local func = MateAuras.LoadFunction("return function() "..(data.actions.init.custom).."\n end", data.id);
        Private.customActionsFunctions[id]["init"] = func
      end
      if data.actions.init.do_custom_load and data.actions.init.customOnLoad then
        local func = MateAuras.LoadFunction("return function() "..(data.actions.init.customOnLoad).."\n end", data.id);
      ...
      if data.actions.init.do_custom_unload and data.actions.init.customOnUnload then
        local func = MateAuras.LoadFunction("return function() "..(data.actions.init.customOnUnload).."\n end", data.id);
    ...
    if (data.actions.start) then
      if (data.actions.start.do_custom and data.actions.start.custom) then
        local func = MateAuras.LoadFunction("return function() "..(data.actions.start.custom).."\n end", data.id);
        Private.customActionsFunctions[id]["start"] = func;
      end

      if (data.actions.start.do_message and data.actions.start.message_custom) then
        local func = MateAuras.LoadFunction("return "..(data.actions.start.message_custom), data.id);
    ...
    if (data.actions.finish) then
      if (data.actions.finish.do_custom and data.actions.finish.custom) then
        local func = MateAuras.LoadFunction("return function() "..(data.actions.finish.custom).."\n end", data.id);
      ...
      if (data.actions.finish.do_message and data.actions.finish.message_custom) then
        local func = MateAuras.LoadFunction("return "..(data.actions.finish.message_custom), data.id);
...
local function CreateTalentCache()
  ...
  if MateAuras.IsClassicOrWrathOrCata() then
  ...
  elseif MateAuras.IsMists() then
    ...
    for tier = 1, MAX_TALENT_TIERS do
      for column = 1, NUM_TALENT_COLUMNS do
        ...
        -- Get the icon and name from the talent cache and record it in the table that will be used by MateAurasOptions
        if (talentName and talentIcon) then
...
-- when using this function, do not name your global data table "MateAurasCompanion"
function MateAuras.AddCompanionData(data)
  MateAuras.DeepMixin(Private.CompanionData, data)
end

-- add data from versions of companion compatible addon that does not use MateAuras.AddCompanionData yet
local function AddLegacyCompanionData()
  local CompanionData = MateAurasCompanion and MateAurasCompanion.MateAuras or MateAurasCompanion
  if CompanionData then
    MateAuras.AddCompanionData(CompanionData)
  end
end
...
function Private.PostAddCompanion()
  ...
  if count and count > 0 then
    MateAuras.prettyPrint(L["There are %i updates to your auras ready to be installed!"]:format(count))
  end
  -- nag if new installs
  if Private.CompanionData.stash and next(Private.CompanionData.stash) then
    MateAuras.prettyPrint(L["You have new auras ready to be installed!"])
  end
end
...
local function tooltip_draw(isAddonCompartment, blizzardTooltip)
  ...
  tooltip:ClearLines()
  tooltip:AddDoubleLine("MateAuras", versionString)
...
MateAuras.GenerateTooltip = tooltip_draw;
...
local Broker_MateAuras;
Broker_MateAuras = LDB:NewDataObject("MateAuras", {
  type = "launcher",
  text = "MateAuras",
  ...
  OnEnter = function(self)
    colorFrame:SetScript("OnUpdate", function(self, elaps)
      ...
      Broker_MateAuras.iconR = r + (r2 - r) * colorElapsed / colorDelay;
      Broker_MateAuras.iconG = g + (g2 - g) * colorElapsed / colorDelay;
      Broker_MateAuras.iconB = b + (b2 - b) * colorElapsed / colorDelay;
    end);
    local elapsed = 0;
    local delay = 1;
...
do -- Archive stuff
  ...
  local function OpenArchive()
    if Archivist:IsInitialized() then
      ...
      if not C_AddOns.IsAddOnLoaded("MateAurasArchive") then
        local ok, reason = C_AddOns.LoadAddOn("MateAurasArchive")
        if not ok then
          reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
          error(string.format(L["Could not load MateAuras Archive, the addon is %s"], reason))
        end
      end
      if type(MateAurasArchive) ~= "table" then
        MateAurasArchive = {}
      end
      Archivist:Initialize(MateAurasArchive)
    end
    return Archivist
  end

  function MateAuras.LoadFromArchive(storeType, storeID)
...
function MateAuras.IsLoginFinished()
    ...
    for i = 1, 10 do
      if (UnitExists ("boss" .. i)) then
        ...
        if (guid and db.CurrentEncounter.boss_guids [guid]) then
          ...
          MateAuras.CurrentEncounter = db.CurrentEncounter
          return true
        end
      end
    end
...
function Private.Login(takeNewSnapshots)
  local loginFunc = (function() -- coroutine.create
    ...
    coroutine.yield(100)
    if db.history then
      local histRepo = MateAuras.LoadFromArchive("Repository", "history")
      local migrationRepo = MateAuras.LoadFromArchive("Repository", "migration")
      ...
      -- history is now in archive so we can shrink MateAurasSaved
      db.history = nil
    end
    ...
    for id, data in pairs(db.displays) do
      if(id ~= data.id) then
        print("|cFF8800FFMateAuras|r detected a corrupt entry in MateAuras saved displays - '"..tostring(id).."' vs '"..tostring(data.id).."'" );
        data.id = id;
      end
...
Private.frames["MateAuras Main Frame"] = MateAurasFrame;
MateAurasFrame:SetAllPoints(UIParent);
...
loadedFrame:SetScript("OnEvent", function(self, event, ...)
  if(event == "ADDON_LOADED") then
    if(... == ADDON_NAME) then
      ...
      -- if not db.disableRuLocale and MateAuras.LoadLocale_ruRU then
      --   MateAuras.LoadLocale_ruRU()
      -- end
      ...
      LDBIcon:Register("MateAuras", Broker_MateAuras, db.minimap);
    end
...
function MateAuras.IsImporting()
  return importing;
end

function MateAuras.IsPaused()
  return paused;
end
...
function MateAuras.InLoadingScreen()
...
function Private.ResumeAllDynamicGroups(suspended)
  for _, id in ipairs(suspended) do
    local region = MateAuras.GetRegion(id)
...
local function StoreBossGUIDs()
  ...
  if (MateAuras.CurrentEncounter and MateAuras.CurrentEncounter.boss_guids) then
    for i = 1, 10 do
      if (UnitExists ("boss" .. i)) then
        local guid = UnitGUID ("boss" .. i)
        if (guid) then
          MateAuras.CurrentEncounter.boss_guids [guid] = true
        end
      end
    end
    db.CurrentEncounter = MateAuras.CurrentEncounter
  end
  Private.StopProfileSystem("boss_guids")
end

local function DestroyEncounterTable()
  if (MateAuras.CurrentEncounter) then
    wipe(MateAuras.CurrentEncounter)
  end
  MateAuras.CurrentEncounter = nil
  db.CurrentEncounter = nil
end
...
local function CreateEncounterTable(encounter_id)
  ...
  return MateAuras.CurrentEncounter
end
...
function MateAuras.InstanceType()
  return (GetInstanceTypeAndSize())
end

---@return string difficulty
function MateAuras.InstanceDifficulty()
  return select(2, GetInstanceTypeAndSize())
end

---@return number? difficultyID
function MateAuras.InstanceTypeRaw()
...
local function scanForLoadsImpl(toCheck, event, arg1, ...)
  ...
  local encounter_id = MateAuras.CurrentEncounter and MateAuras.CurrentEncounter.id or 0
  ...
  if MateAuras.IsClassicOrWrathOrCataOrMists() then
    local raidID = UnitInRaid("player")
    if raidID then
      raidRole = select(10, GetRaidRosterInfo(raidID))
    end
    role = "none"
    if MateAuras.IsWrathClassic() then
  ...
  if MateAuras.IsClassicEra() then
    vehicle = UnitOnTaxi('player')
  end
  if MateAuras.IsWrathOrCataOrMistsOrRetail() then
  ...
  if MateAuras.IsMistsOrRetail() then
  ...
  if (MateAuras.CurrentEncounter) then
    if (instanceId ~= MateAuras.CurrentEncounter.zone_id and not inCombat) then
      encounter_id = 0
      DestroyEncounterTable()
    end
  end
  ...
  local addonRestrictionsActive = MateAuras.IsSecretStateActive()
  ...
  for id in pairs(toCheck) do
    local data = MateAuras.GetData(id)
    if (data and not data.controlledChildren) then
      ...
      -- elseif MateAuras.IsWrathClassic() then
      ...
      -- elseif MateAuras.IsCataClassic() then
...
if MateAuras.IsWrathOrCataOrMists() then
  loadFrame:RegisterEvent("VEHICLE_UPDATE");
  loadFrame:RegisterEvent("UPDATE_VEHICLE_ACTIONBAR")
  loadFrame:RegisterEvent("UPDATE_OVERRIDE_ACTIONBAR");
end

if MateAuras.IsMists() then
  loadFrame:RegisterEvent("PET_BATTLE_OPENING_START");
  loadFrame:RegisterEvent("PET_BATTLE_CLOSE");
end
...
function Private.LoadDisplays(toLoad, ...)
  for id in pairs(toLoad) do
    ...
    if Private.DebugLog.IsEnabled(uid) then
      MateAuras.prettyPrint(L["Debug Logging enabled for '%s'"]:format(id))
...
function Private.GetDataByUID(uid)
  return MateAuras.GetData(UIDtoID[uid])
end
...
---@private
function MateAuras.Delete(data)
...
function MateAuras.Rename(data, newid)
  ...
  if(data.parent) then
    ...
    local parentRegion = MateAuras.GetRegion(data.parent)
...
function MateAuras.DeepMixin(dest, source)
...
function Private.NeedToRepairDatabase()
  return db.dbVersion and db.dbVersion > MateAuras.InternalVersion()
end
...
local function RepairDatabase()
  local func = (function()
    ...
    db.dbVersion = MateAuras.InternalVersion()
...
StaticPopupDialogs["MateAuras_CONFIRM_REPAIR"] = {
  text = "",
  button1 = L["Repair"],
  button2 = L["Cancel"],
  ...
  for _, data in pairs(db.displays) do
    if type(data.uid) == "string" then
      if seenUIDs[data.uid] then
        ...
        data.uid = MateAuras.GenerateUniqueID()
...
function Private.CheckForAnchorCycle(source)
  ...
  while source do
    cycle[source] = true
    local data = MateAuras.GetData(source)
    local target
    if data then
...
function Private.AddMany(tbl, takeSnapshots)
  ...
  for _, source in pairs(anchorTargets) do
    ...
    if Private.CheckForAnchorCycle(source) then
      MateAuras.prettyPrint(L["Warning: Anchoring in aura '%s' is imposssible, due to an anchoring cycle"]:format(source))
  ...
  if takeSnapshots then
    ...
    if #order > 0 then
      Private:Async({name = "snapshot"}, function()
        prettyPrint(L["MateAuras is creating a rollback snapshot of your auras. This snapshot will allow you to revert to the current state of your auras if something goes wrong. This process may cause your framerate to drop until it is complete."])
      ...
      if next(MateAuras.LoadFromArchive("Repository", "migration").stores) ~= nil then
        C_Timer.After(1, function()
          prettyPrint(L["MateAuras has detected empty settings. If this is unexpected, ask for assitance on https://discord.gg/MateAuras."])
        end)
      end
    end
  end
  ...
  for _, data in ipairs(order) do
    if data.parent and bads[data.parent] then
      ...
      local ok = xpcall(MateAuras.PreAdd, Private.GetErrorHandlerUid(data.uid, "PreAdd"), data, oldSnapshot)
      if not ok then
        prettyPrint(L["Unable to modernize aura '%s'. This is probably due to corrupt data or a bad migration, please report this to Mate-R."]:format(data.id))
    ...
    if not customOptionIsValid(option) or authorOptionKeys[option.key] then
      ...
      if optionClass == "simple" then
        if not option.key then
          option.key = MateAuras.GenerateUniqueID()
        end
...
function Private.ClearSounds(uid, severity)
  ...
  MateAuras.ClearAndUpdateOptions(data.id, true)
  MateAuras.FillOptions()
end

function MateAuras.PreAdd(data, snapshot)
    ...
    for _, subRegionData in ipairs(data.subRegions) do
      ...
      if subType and Private.subRegionTypes[subType] then
        if Private.subRegionTypes[subType].supports(data.regionType) then
          ...
          MateAuras.prettyPrint(L["ERROR in '%s' unknown or incompatible sub element type '%s'"]:format(data.id, subType))
        end
      end
    end
  end
...
function pAdd(data, simpleChange)
  local id = data.id;
  if not(id) then
    error("Improper arguments to MateAuras.Add - id not defined");
    return;
  end

  data.uid = data.uid or MateAuras.GenerateUniqueID()
  if db.displays[id] and db.displays[id].uid ~= data.uid then
    print("Improper? arguments to MateAuras.Add - id", id, "is assigned to a different uid.", data.uid, db.displays[id].uid)
  end
  if UIDtoID[data.uid] and UIDtoID[data.uid] ~= id then
    print("Improper? arguments to MateAuras.Add - uid is assigned to a id. Uid:", data.uid, "assigned too:", UIDtoID[data.uid], "assigning now to", data.id)
  end
  ...
  if simpleChange then
    db.displays[id] = data
    cycleCheck(data)
    if MateAuras.GetRegion(data.id) then
    ...
    if Private.IsGroupType(data) then
      ...
      if data.triggers.disjunctive == "custom" then
        triggerLogicFunc = MateAuras.LoadFunction("return "..(data.triggers.customTriggerLogic or ""), data.id);
      end
...
function MateAuras.Add(data, simpleChange)
  Private.TimeMachine:DestroyTheUniverse(data.id)
...
function Private.AddParents(data)
  ...
  if (parent) then
    local parentData = MateAuras.GetData(parent)
    MateAuras.Add(parentData)
...
function Private.SetRegion(data, cloneId)
  ...
  if not(regionType) then
    ...
    if(not regionTypes[regionType]) then
      regionType = "fallback";
      print("Improper arguments to MateAuras.CreateRegion - regionType \""..data.regionType.."\" is not supported in ".. data.id)
    end
    ...
    if not(id) then
      ...
      if(cloneId) then
        ...
        if (not region or region.regionType ~= data.regionType) then
          ...
          if(clonePool[data.regionType] and clonePool[data.regionType][1]) then
            ...
            local clone = regionTypes[data.regionType].create(MateAurasFrame, data);
        ...
        if((not Private.regions[id]) or (not Private.regions[id].region) or Private.regions[id].regionType ~= regionType) then
          region = regionTypes[regionType].create(MateAurasFrame, data);
          Private.regions[id] = {
      ...
      local loginFinished = MateAuras.IsLoginFinished();
...
local function EnsureRegion(id)
  if not Private.regions[id] or not Private.regions[id].region then
    ...
    while(id) do
      ...
      if MateAuras.GetRegion(id) then
        break
      end
    end

    for _, toCreateId in ipairs_reverse(aurasToCreate) do
      local data = MateAuras.GetData(toCreateId)
      Private.SetRegion(data)
    end
  end
...
---@return table|nil
function MateAuras.GetRegion(id, cloneId)
  if(cloneId and cloneId ~= "") then
...
function Private.SetAllStatesHidden(id, triggernum)
  local triggerState = MateAuras.GetTriggerStateForTrigger(id, triggernum);
  local changed = false
...
function Private.ReleaseClone(id, cloneId, regionType)
  ...
  if region:IsProtected() then
    MateAuras.prettyPrint(L["Error '%s' created a secure clone. We advise deleting the aura. For more information:\nhttps://github.com/Mate-R/MateAuras/wiki/Protected-Frames"]:format(id))
  else
...
do
  local function frame_monitor_callback(event, frame, unit, previousUnit)
      ...
      for region, data in pairs(glow_frame_monitor) do
        ...
        then
          if not new_frame then
            new_frame = MateAuras.GetUnitFrame(unit)
          end
    ...
    if type(anchor_unitframe_monitor) == "table" then
      for region, data in pairs(anchor_unitframe_monitor) do
        ...
        then
          if not new_frame then
            new_frame = MateAuras.GetUnitFrame(unit) or MateAuras.HiddenFrames
          end
          if new_frame ~= data.frame then
  ...
  LGF.RegisterCallback("MateAuras", "FRAME_UNIT_UPDATE", frame_monitor_callback)
  LGF.RegisterCallback("MateAuras", "FRAME_UNIT_ADDED", frame_monitor_callback)
  LGF.RegisterCallback("MateAuras", "FRAME_UNIT_REMOVED", frame_monitor_callback)
end
...
function Private.HandleGlowAction(actions, region)
  ...
  then
    ...
    elseif actions.glow_frame_type == "UNITFRAME" and region.state.unit then
      glow_frame = MateAuras.GetUnitFrame(region.state.unit)
      should_glow_frame = true
    elseif actions.glow_frame_type == "NAMEPLATE" and region.state.unit then
      glow_frame = MateAuras.GetUnitNameplate(region.state.unit)
      should_glow_frame = true
...
function Private.PerformActions(data, when, region)
  if (paused or MateAuras.IsOptionsOpen()) then
    return;
  end;
  local actions;
  local formatters
...
-- This has to be in MateAuras for now, because GetNameAndIcon can be called from the options
-- before the Options has access to Private
MateAuras.GetNameAndIcon = wrapTriggerSystemFunction("GetNameAndIcon", "nameAndIcon");
...
do
  local hiddenTooltip;
  function MateAuras.GetHiddenTooltip()
    if not(hiddenTooltip) then
      hiddenTooltip = CreateFrame("GameTooltip", "MateAurasTooltip", nil, "GameTooltipTemplate");
...
function MateAuras.GetAuraInstanceTooltipInfo(unit, auraInstanceId, filter)
  if MateAuras.IsRetail() then
    local tooltipText = ""
    local tooltipData
...
function MateAuras.GetAuraTooltipInfo(unit, index, filter)
  local tooltipText = ""
  if MateAuras.IsRetail() then
    ...
    local tooltip = MateAuras.GetHiddenTooltip();
    tooltip:ClearLines();
...
local FrameTimes = {};
function MateAuras.ProfileFrames(all)
  UpdateAddOnCPUUsage();
...
local DisplayTimes = {};
function MateAuras.ProfileDisplays(all)
  UpdateAddOnCPUUsage();
...
local function FixGroupChildrenOrderImpl(data, frameLevel)
  ...
  for _, childId in ipairs(data.controlledChildren) do
    local childData = MateAuras.GetData(childId)
...
function MateAuras.EnsureString(input)
  if (input == nil) then
    return "";
  end
...
function MateAuras.RegisterTriggerSystem(types, triggerSystem)
...
function MateAuras.RegisterTriggerSystemOptions(types, func)
  for _, v in ipairs(types) do
    Private.triggerTypesOptions[v] = func;
  end
end

function MateAuras.GetTriggerStateForTrigger(id, triggernum)
...
function MateAuras.GetActiveStates(id)
  return triggerState[id].activeStates
end

function MateAuras.GetActiveTriggers(id)
...
do
  ...
  function Private.UpdateFakeStatesFor(id)
    if (MateAuras.IsOptionsOpen() and visibleFakeStates[id]) then
      local data = MateAuras.GetData(id)
      if (data) then
        ...
        Private.UpdatedTriggerState(id)
        if MateAuras.GetMoverSizerId() == id then
          MateAuras.SetMoverSizer(id)
        end
      end
    end
  end
    ...
    if (state.show) then
      ...
      for triggernum = -1, triggerState[id].numTriggers do
        ...
        if triggernum == activeTrigger then
          ...
          local triggerStates = MateAuras.GetTriggerStateForTrigger(id, triggernum)
...
function Private.UpdatedTriggerState(id)
  ...
  local activeTriggerState = MateAuras.GetTriggerStateForTrigger(id, newActiveTrigger);
...
function Private.RunCustomTextFunc(region, customFunc)
  ...
  if state then
    if state.progressType == "durationObject" and MateAuras.IsDurationObject(state.durationObject) then
...
local function ReplaceValuePlaceHolders(textStr, region, customCache, state, formatter, trigger)
  ...
  if customIndexSubStr then
    ...
    if custom then
      value = MateAuras.EnsureString(value)
    end
  else
...
function MateAuras.IsAuraActive(id)
  local active = triggerState[id]
...
local function ensureMouseFrame()
  ...
  ---@class Frame
  mouseFrame = CreateFrame("Frame", "MateAurasAttachToMouseFrame", UIParent);
  mouseFrame.attachedVisibleFrames = {};
  mouseFrame:SetWidth(1);
  mouseFrame:SetHeight(1);

  local moverFrame = CreateFrame("Frame", "MateAurasMousePointerFrame", mouseFrame);
  ...
  label:SetText("MateAuras Anchor");
...
function Private.ensurePRDFrame()
  ...
  personalRessourceDisplayFrame = CreateFrame("Frame", "MateAurasAttachToPRD", UIParent);
  ...
  local moverFrame = CreateFrame("Frame", "MateAurasPRDMoverFrame", personalRessourceDisplayFrame);
  ...
  texture:SetAllPoints(moverFrame);
  texture:SetTexture("Interface\\AddOns\\MateAuras\\Media\\Textures\\PRDFrame");
  ...
  personalRessourceDisplayFrame.OptionsOpened = function()
    ...
    if (KuiNameplatesCore and KuiNameplatesCore.profile) then
      ...
      personalRessourceDisplayFrame.texture:SetTexture("Interface\\AddOns\\MateAuras\\Media\\Textures\\PRDFrameKui");
    else
      ...
      -- personalRessourceDisplayFrame.texture:SetTexture("Interface\\AddOns\\MateAuras\\Media\\Textures\\PRDFrame");
    end
...
local function tryAnchorAgain()
  ...
  for id, _ in pairs(delayed) do
    ...
    if (data and region) then
      ...
      local parentData
      if data.parent then
        parentData = MateAuras.GetData(data.parent)
...
local HiddenFrames = CreateFrame("Frame", "MateAurasHiddenFrames")
HiddenFrames:Hide()
MateAuras.HiddenFrames = HiddenFrames
...
local function GetAnchorFrame(data, region, parent)
  ...
  if (anchorFrameType == "NAMEPLATE") then
    ...
    if unit then
      local frame = unit and MateAuras.GetUnitNameplate(unit)
  ...
  if (anchorFrameType == "UNITFRAME") then
    local unit = region.state and region.state.unit
    if unit then
      local frame = MateAuras.GetUnitFrame(unit) or MateAuras.HiddenFrames
      if frame then
  ...
  if (anchorFrameType == "CUSTOM" and region.customAnchorFunc) then
    ...
    if ok and frame then
      return frame
    elseif MateAuras.IsOptionsOpen() then
      return parent
    else
      return HiddenFrames
    end
...
function Private.AnchorFrame(data, region, parent, force)
  ...
  and not MateAuras.IsLoginFinished()
  and not force
  then
    ...
    if (data.anchorFrameParent or data.anchorFrameParent == nil
      ...
      region:SetParent(parent or MateAurasFrame);
    end
...
function MateAuras.SetModel(frame, unused, model_fileId, isUnit, isDisplayInfo)
...
function MateAuras.SafeToNumber(input)
...
do
  ...
  function MateAuras.IsUntrackableSoftTarget(unit)
  ...
  ---@param unit UnitToken
  ---@return boolean result
  function MateAuras.UntrackableUnit(unit)
...
do
  ...
  ---@return string name
  ---@return string realm
  function MateAuras.UnitNameWithRealm(unit)
    ownRealm = ownRealm or select(2, UnitFullName("player"))
    local name, realm = UnitFullName(unit)
    return name or "", realm or ownRealm or ""
  end

  function MateAuras.UnitNameWithRealmCustomName(unit)
    ownRealm = ownRealm or select(2, UnitFullName("player"))
    local name, realm =  MateAuras.UnitFullName(unit)
    return name or "", realm or ownRealm or ""
  end
end
...
function Private.ExecEnv.ParseBossModCheck(input)
  ...
  -- check if it is enabled with MateAuras.IsBossModEnabled(encounterId)

  local matcher = {
    encounterIds = {},
    Check = function(self)
      return MateAuras.IsBossModEnabled(self.encounterIds)
    end,
...
function MateAuras.IsAuraLoaded(id)
...
-- are categorized
---@private
function MateAuras.GetTriggerCategoryFor(triggerType)
...
function Private.SortOrderForValues(values)
  ...
  table.sort(sortOrder, function(aKey, bKey)
    ...
    if type(aValue) == "string" and aValue:sub(1, #MateAuras.newFeatureString) == MateAuras.newFeatureString then
      aValue = aValue:sub(#MateAuras.newFeatureString + 1)
    end

    if type(bValue) == "string" and bValue:sub(1, #MateAuras.newFeatureString) == MateAuras.newFeatureString then
      bValue = bValue:sub(#MateAuras.newFeatureString + 1)
    end
...
do
  ...
  local function Traverse(data, includeSelf, includeGroups, includeLeafs)
    ...
    if data.controlledChildren then
      for _, child in ipairs(data.controlledChildren) do
        Traverse(MateAuras.GetData(child), true, includeGroups, includeLeafs)
      end
    end
  end
  ...
  local function TraverseParents(data)
    while data.parent do
      local parentData = MateAuras.GetData(data.parent)
