...
local MAJOR_VERSION = "LibCustomGlow-1.0-mateauras"
...
function lib.PixelGlow_Start(r,color,N,frequency,length,th,xOffset,yOffset,border,key,frameLevel)
    if not r or (r.GetSize and issecretvalue(r:GetSize())) then
        return
    end
    if not color then
    ...
    if r._ButtonGlow then
        ...
        if not(color) then
            for texture in pairs(ButtonGlowTextures) do
                ...
                local frameAlpha = f[texture]:GetAlpha()
                if issecretvalue(frameAlpha) then
                    frameAlpha = 1
                end
                local alpha = math.min(frameAlpha/noZero(f.color and f.color[4] or 1), 1)
            ...
            for texture in pairs(ButtonGlowTextures) do
                ...
                local alpha = math.min(frameAlpha/noZero(f.color and f.color[4] or 1)*color[4], 1)
                f[texture]:SetAlpha(alpha)
