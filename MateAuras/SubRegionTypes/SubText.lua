...
local defaultFont = MateAuras.defaultFont
local defaultFontSize = MateAuras.defaultFontSize
...
local function create()
  ...
  local fontObject = CreateFont("MateAuras-SubText-Font" .. fontObjectCounter)
...
local function modify(parent, region, parentData, data, first)
  ...
  if text:GetFont() then
    ...
    text:SetText(MateAuras.ReplaceRaidMarkerSymbols(data.text_text));
  end
  ...
  if first then
    ...
    if containsCustomText and parentData.customText and parentData.customText ~= "" then
      parent.customTextFunc = MateAuras.LoadFunction("return "..parentData.customText, parentData.id)
  ...
  function region:ConfigureTextUpdate()
    ...
    if region.text_text and Private.ContainsAnyPlaceHolders(region.text_text) then
      UpdateText = function()
        ...
        if text:GetFont() then
          text:SetText(MateAuras.ReplaceRaidMarkerSymbols(textStr))
        end
...
MateAuras.RegisterSubRegionType("subtext", L["Text"], supports, create, modify, onAcquire, onRelease,
