if not MateAuras.IsLibsOK() then
  return
end
---@type string
local AddonName = ...
...
MateAurasProfilingReportMixin = {}
function MateAurasProfilingReportMixin:OnShow()
  ...
  self:SetTitle(L["MateAuras Profiling Report"])
  self:SetSize(500, 300)
end

function MateAurasProfilingReportMixin:ClearText()
  self.ScrollBox.messageFrame:SetText("")
end

function MateAurasProfilingReportMixin:AddText(v)
  if not v then
    return
  end
  --- @type string?
...
local RegisterProfile = function(startType)
  ...
  if startType == "encounter" then
    MateAurasProfilingFrame:UnregisterAllEvents()
    prettyPrint(L["Your next encounter will automatically be profiled."])
    MateAurasProfilingFrame:RegisterEvent("ENCOUNTER_START")
    MateAurasProfilingFrame:RegisterEvent("ENCOUNTER_END")
  ...
  elseif startType == "combat" then
    ...
    MateAurasProfilingFrame:RegisterEvent("PLAYER_REGEN_DISABLED")
    MateAurasProfilingFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
    currentProfileState = startType
    delayedStart = true
  ...
  elseif startType and startType:match("%d") then
    ...
    ProfilingTimer = MateAuras.timer:ScheduleTimer(MateAuras.StopProfile, time)
    currentProfileState = "profiling"
  else
    MateAurasProfilingFrame:UnregisterAllEvents()
  ...
  MateAurasProfilingFrame:UpdateButtons()
  return delayedStart
end

---@diagnostic disable-next-line: duplicate-set-field
function MateAuras.StartProfile(startType)
...
function MateAuras.StopProfile()
  ...
  if MateAurasProfilingFrame then
    MateAurasProfilingFrame:UnregisterAllEvents()
    MateAurasProfilingFrame:UpdateButtons()
  end

  if ProfilingTimer then
    MateAuras.timer:CancelTimer(ProfilingTimer)
    ProfilingTimer = nil
  end
end

function MateAuras.ToggleProfile()
  if not profileData.systems.time or profileData.systems.time.count ~= 1 then
    MateAuras.StartProfile()
  else
...
local function ColoredSpike(spike)
  local r, g, b
  if spike < 2 then
    r, g, b = MateAuras.GetHSVTransition(spike / 2, 0, 1, 0, 1, 1, 1, 0, 1)
  elseif spike < 2.5 then
    r, g, b = MateAuras.GetHSVTransition((spike - 2) * 2, 1, 1, 0, 1, 1, 0.65, 0, 1)
  elseif spike < 3 then
    r, g, b = MateAuras.GetHSVTransition((spike - 2.5) * 2, 1, 0.65, 0, 1, 1, 0, 0, 1)
  else
    r, g, b = 1, 0, 0
  end
...
function MateAuras.PrintProfile()
  local popup = MateAurasProfilingReport
  ...
  if MateAurasProfilingFrame and MateAurasProfilingFrame:IsShown() then
    popup:SetParent(MateAurasProfilingFrame)
    popup:SetPoint("TOPLEFT", MateAurasProfilingFrame, "TOPRIGHT", 5, 0)
  else
    popup:SetParent(UIParent)
    if MateAurasSaved.ProfilingWindow then
      popup:SetPoint("TOPLEFT", UIParent, "TOPLEFT", MateAurasSaved.ProfilingWindow.xOffset or 0, MateAurasSaved.ProfilingWindow.yOffset or 0)
    else
  ...
  popup:AddText("You can ask on our discord https://discord.gg/MateAuras for help interpreting this output.")
...
function MateAurasProfilingMixin:OnShow()
  ...
  ButtonFrameTemplate_HidePortrait(self)
  self:SetTitle(L["MateAuras Profiling"])
  ...
  self.buttons.report.tooltip = L["A detailed overview of your auras and MateAuras systems\nCopy the whole text to Weakaura's Discord if you need assistance."]
  ...
  view:SetElementInitializer("MateAurasProfilingLineTemplate", function(frame, elementData)
...
function MateAurasProfilingStopButton_OnClick(self)
  if currentProfileState == "profiling" then
    MateAuras.StopProfile()
  else
...
function MateAurasProfilingMixin:InitDropDown()
...
function MateAurasProfilingMixin:InitModeDropDown()
  local function Initializer(dropDown, level)
    for i = 1, 2 do
      local info = UIDropDownMenu_CreateInfo()
      info.text = modes[i]
...
local lastRefresh
function MateAurasProfilingMixin:RefreshBars(_, force)
...
function MateAurasProfilingMixin:ResetBars()
  self.bars:Flush()
end

function MateAurasProfilingMixin:UpdateBar(name, time, pct, spike)
  ...
  if elementData then
    ...
    local button = MateAurasProfilingFrame.ScrollBox:FindFrame(elementData)
    if button then
...
function MateAurasProfilingMixin:SortByColumnIndex(index)
...
function MateAurasProfilingMixin:UpdateButtons()
...
function MateAurasProfilingMixin:Start()
  self:Show()
end

function MateAurasProfilingMixin:Stop()
  MateAuras.StopProfile()
  self:UpdateButtons()
  self:ResetBars()
end

function MateAurasProfilingMixin:Toggle()
...
function MateAurasProfilingColumnDisplay_OnClick(self, columnIndex)
