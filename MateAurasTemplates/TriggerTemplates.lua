...
local function createDurationTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = MateAuras.GetTriggerCategoryFor("Combat Log"),
      event = "Combat Log",
...
local function createTotemTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = MateAuras.GetTriggerCategoryFor("Totem"),
      event = "Totem",
...
local function createPowerTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = MateAuras.GetTriggerCategoryFor("Power"),
      event = "Power",
      use_unit = true,
      unit = "player",
      use_powertype = true,
...
local function createHealthTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = MateAuras.GetTriggerCategoryFor("Health"),
      event = "Health",
      unit = "player",
      use_unit = true,
      use_absorbMode = true,
...
local function createCastTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = MateAuras.GetTriggerCategoryFor("Cast"),
      event = "Cast",
      use_unit = true,
...
local function createAbilityTrigger(triggers, position, item, genericShowOn)
  triggers[position] = {
    trigger = {
      type = MateAuras.GetTriggerCategoryFor("Cooldown Progress (Spell)"),
...
local function createItemTrigger(triggers, position, item, genericShowOn)
  triggers[position] = {
    trigger = {
      type = MateAuras.GetTriggerCategoryFor("Cooldown Progress (Item)"),
...
local function createOverlayGlowTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = MateAuras.GetTriggerCategoryFor("Spell Activation Overlay"),
...
local function createWeaponEnchantTrigger(triggers, position, item, showOn)
  triggers[position] = {
    trigger = {
      type = MateAuras.GetTriggerCategoryFor("Weapon Enchant"),
...
local function createQueuedActionTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = MateAuras.GetTriggerCategoryFor("Queued Action"),
      event = "Queued Action",
      spellName = item.spell
    }
  }
...
local function subTypesFor(item, regionType)
  ...
  local icon = {
    ...
    glow = function()
      ...
      t1:SetTexture(134376);
      t1:SetAllPoints(thumbnail);
      MateAuras.ShowOverlayGlow(thumbnail); -- where to call HideOverlayGlow() ?
      return thumbnail;
    end, -- 571554
    charges = function()
      local thumbnail = createThumbnail();
...
function MateAuras.CreateTemplateView(Private, frame)
  ...
  local function createRegionFlyout(regionType, regionData)
    local group = AceGUI:Create("MateAurasTemplateGroup");
    group:SetFullWidth(true);
    group:SetLayout("WATemplateTriggerLayoutFlyout");
    for _, item in ipairs(regionData.templates) do
      local templateButton = AceGUI:Create("MateAurasNewButton");
      if (item.icon) then
      ...
      templateButton:SetClick(function()
        ...
        newView.data.internalVersion = MateAuras.InternalVersion();
            ...
            if #subTypes < 2 then
              ...
              if (newView.existingAura) then
                ...
                if (subType.data) then
                  MateAuras.DeepMixin(newView.data, subType.data)
                end
                newView:CancelClose();
                MateAuras.NewAura(newView.data, newView.data.regionType, newView.targetId);
              end
            else
  ...
  local function createLastPage()
    local replaceButton = AceGUI:Create("MateAurasNewButton");
    ...
    replaceButton:SetClick(function()
      ...
      if (newView.chosenItem.load) then
        MateAuras.DeepMixin(newView.data.load, newView.chosenItem.load)
      end
    end);
    newViewScroll:AddChild(replaceButton);

    local addButton = AceGUI:Create("MateAurasNewButton");
  ...
  createButtons = function(selectedItem) -- selectedItem is either a regionType or a trigger section
    ...
    elseif (newView.data and not newView.chosenItem) then
      ...
      -- Class
      local classSelector = createDropdown("class", MateAuras.class_types);
      newViewScroll:AddChild(classSelector);

      if MateAuras.IsRetail() then
        local specSelector = createDropdown("spec", MateAuras.spec_types_specific[newView.class]);
      ...
      local raceSelector = createDropdown("race", MateAuras.race_types);
