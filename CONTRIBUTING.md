...
- All user-facing strings (`names` and `desc` fields in AceConfig tables, mostly) must be localized:
  - We use a locale scraper to find translation phrases and automatically export them to CurseForge for translation. This scraper parses the addon files, looking for tokens that look like: `L["some translation phrase"]`. You must use double quoted strings, and name the localization table (found at `MateAuras.L`) `L` in your code for this to work properly.
...
- New features should be indicated by concatenating `MateAuras.newFeatureString` onto the associated translation phrase. We will remove the new feature indicator approximately 3 months after the first release.
...
1. [Fork](https://github.com/Mate-R/MateAuras/fork) our repository.
...
1. Take a look at our [Wiki](https://github.com/Mate-R/MateAuras/wiki/Lua-Dev-Environment) page on how to setup a Lua dev environment.
...
1. [Open a Pull Request](https://github.com/Mate-R/MateAuras/pulls) with a clear title and description.
...
- Specify a new remote upstream repository that will be used to sync your fork (you only need to do this once).
  ...
  ```bash
  git remote add upstream https://github.com/Mate-R/MateAuras.git
  ```

- In order to sync your fork with the upstream MateAuras repository you would do
...
- In order to sync your pull request with the upstream MateAuras repository in case there are any conflicts you would do
...
1. Please check our [issue tracker](https://github.com/Mate-R/MateAuras/issues) for your problem since there's a good
...
1. If you didn't find any tickets with a problem similar to yours then please open a
   [new ticket](https://github.com/Mate-R/MateAuras/issues/new/choose).
